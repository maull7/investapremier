                                ...
                                <tr class="hover:bg-[#f8fafc] transition">
                                    ...
                                    <td class="px-5 py-3.5">
                                        ...
                                        @elseif ($profile?->profil_risiko)
                                            <span class="px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600">{{ $profile->profil_risiko }}</span>
                                        @else
                                            <span class="text-muted text-xs">—</span>
                                        @endif
                                    </td>
                                    <td class="px-5 py-3.5 text-right font-semibold text-primary">{{ $latestPlan?->dana_tersedia ? 'Rp ' . number_format($latestPlan->dana_tersedia, 0, ',', '.') : '—' }}</td>
                                    ...
                                    <td class="px-5 py-3.5 text-right">
                                        <a href="{{ route('user.clients.show', $c) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-accent border border-accent/30 rounded-lg hover:bg-accent/5 transition">
                        ...
                        <tbody class="divide-y divide-line">
                            @foreach ($pendingRequests as $req)
                                ...
                                <tr class="hover:bg-[#f8fafc] transition">
                                    ...
                                    <td class="px-5 py-3.5">
                                        @if ($p)
                                            <span class="px-2 py-0.5 rounded-full text-xs font-medium {{ $p->status === 'approved' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700' }}">{{ $p->status === 'approved' ? 'Aktif' : 'Pending' }}</span>
                                        @else
                                            <span class="text-muted text-xs">—</span>
                                        @endif
                                    </td>
                        ...
                        <tbody class="divide-y divide-line">
                            @foreach ($rejectedRequests as $req)
                                ...
                                <tr class="hover:bg-[#f8fafc] transition">
                                    ...
                                    <td class="px-5 py-3.5 text-muted text-xs">{{ $req->client->email }}</td>
                                    <td class="px-5 py-3.5 text-muted">{{ $p?->pekerjaan ?? '—' }}</td>
                                    <td class="px-5 py-3.5">{{ $p?->profil_risiko ? ucfirst($p->profil_risiko) : '—' }}</td>
                                    <td class="px-5 py-3.5"><span class="px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-700">Ditolak</span></td>
                                    <td class="px-5 py-3.5 text-right">
                                        <form method="POST" action="{{ route('user.clients.destroy', $req->client) }}" class="inline" onsubmit="return confirm('Hapus dari daftar?')">
                                            @csrf @method('DELETE')
