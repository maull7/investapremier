...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
    <div class="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
        ...
        <div class="space-y-4">
            ...
            <div>
                <div class="flex justify-between text-sm mb-1.5">
                    <span class="font-500 text-gray-700">{{ $item['label'] }}</span>
                    <span class="font-700 text-gray-900 font-bold">{{ $item['pct'] }}%</span>
                </div>
                <div class="prog-track">
                    <div class="prog-fill bg-gradient-to-r {{ $item['warna'] }}" style="width:{{ $item['pct'] }}%"></div>
                </div>
            </div>
            @endforeach
        </div>
        ...
        <div class="text-center py-8">
            ...
            <a href="{{ route('user.perencanaan-investasi.create') }}" class="btn-ip btn-green mt-3 inline-flex">Buat Rencana Investasi</a>
        </div>
        @endif
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
        ...
        <div class="space-y-5">
            ...
            <div>
                <div class="flex justify-between text-sm mb-1">
                    <span class="font-600 text-gray-800 font-semibold">{{ $goal['nama'] }}</span>
        ...
        <div class="text-center py-8">
            ...
            <a href="{{ route('user.perencanaan-investasi.create') }}" class="btn-ip btn-green mt-3 inline-flex">Buat Goal Baru</a>
        </div>
        @endif
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
    <div class="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
        ...
        <div class="space-y-3">
            ...
            <div class="flex items-start gap-3 p-3 rounded-xl bg-{{ $alert['bgColor'] }}-50 border border-{{ $alert['borderColor'] }}-100 text-{{ $alert['textColor'] }}-700">
                ...
                <span class="text-sm leading-relaxed">{{ $alert['message'] }}</span>
            </div>
            @endforeach
        </div>
        @else
        <div class="text-center py-8">
            <p class="text-sm text-gray-400">Tidak ada alert.</p>
        </div>
        @endif
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
        ...
        <div class="rounded-xl p-4 border border-green-100" style="background:linear-gradient(135deg,#f0fdf4,#dcfce7)">
            <div class="flex items-start gap-3">
                <div class="w-9 h-9 rounded-full flex-shrink-0 text-white text-xs font-bold grid place-items-center" style="background:linear-gradient(135deg,#16a34a,#22c55e)">{{ $advisor['initial'] }}</div>
                <div>
                    <div class="text-sm font-semibold text-gray-900">{{ $advisor['name'] }}</div>
                    <p class="text-sm text-gray-600 mt-1 leading-relaxed">Advisor Anda siap membantu. Hubungi untuk konsultasi portfolio.</p>
        ...
        <div class="rounded-xl p-4 border border-gray-100 bg-gray-50 text-center">
            ...
            <a href="{{ route('user.clients.requests.index') }}" class="text-sm font-semibold text-green-600 hover:underline mt-2 inline-block">Cari Advisor</a>
        </div>
        @endif
    </div>
