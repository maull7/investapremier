...
@section('content')
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-accent-teal/85">Manajer Investasi</h1>
        <p class="page-sub">Data AUM dan UP manajer investasi per periode</p>
    </div>
                            ...
                            <tbody class="divide-y divide-line">
                                @foreach ($m->periods->sortBy('period_date') as $p)
                                    <tr class="hover:bg-[#f8fafc] transition-colors">
                                        <td class="px-4 py-3">
                                            <span
                                                class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-primary/5 text-primary font-semibold text-xs">
        ...
        <div class="flex items-center justify-between gap-4 text-sm">
            ...
            @if ($managers->hasPages())
                <div class="flex items-center gap-1">
                    ...
                    @else
                        <a href="{{ $managers->previousPageUrl() }}"
                    ...
                    @foreach ($managers->getUrlRange(1, $managers->lastPage()) as $page => $url)
                        <a href="{{ $url }}"
                            class="w-8 h-8 rounded-lg grid place-items-center text-xs font-semibold transition {{ $page == $managers->currentPage() ? 'bg-accent-teal/85 text-white' : 'text-muted hover:text-primary hover:bg-[#f1f5f9]' }}">{{ $page }}</a>
                    @endforeach
                    @if ($managers->hasMorePages())
                        <a href="{{ $managers->nextPageUrl() }}"
