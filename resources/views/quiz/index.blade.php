...
@section('content')
    ...
    @if (!request()->boolean('start'))
        ...
        <div class="max-w-2xl mx-auto">
            <div class="bg-white rounded-2xl border border-line shadow-sm overflow-hidden">
                {{-- Header --}}
                <div class="bg-gradient-to-r from-accent-teal to-accent-dark px-8 py-8 text-white text-center">
                ...
                <div class="px-8 py-6">
                    ...
                    <div class="mb-6">
                        ...
                        <ul class="space-y-2 text-sm text-muted">
                            <li class="flex items-start gap-2">
                                ...
                                Pilih <strong class="text-primary">satu jawaban</strong> yang paling sesuai dengan kondisi
                                dan preferensi Anda
                            </li>
                            <li class="flex items-start gap-2">
                                <svg class="w-4 h-4 text-accent shrink-0 mt-0.5" fill="none" stroke="currentColor"
                                ...
                                Jawab dengan <strong class="text-primary">jujur</strong> agar hasil profil mencerminkan
                                kondisi sebenarnya
                            </li>
                            <li class="flex items-start gap-2">
                                <svg class="w-4 h-4 text-accent shrink-0 mt-0.5" fill="none" stroke="currentColor"
                            ...
                            @if ($result)
                                <li class="flex items-start gap-2">
                                    ...
                                    Mengisi ulang akan <strong class="text-primary">mengganti</strong> hasil profil
                                    sebelumnya (saat ini: <strong class="text-primary">{{ $result->profile }}</strong>)
                                </li>
                            @endif
                        </ul>
                    </div>
                    ...
                    <div class="mb-6 p-4 bg-[#f8fafc] rounded-xl border border-line">
                        <p class="text-xs font-semibold text-muted uppercase tracking-wide mb-3">Hasil Profil yang Mungkin
                            Anda Dapatkan</p>
                        <div class="grid grid-cols-2 gap-2">
                            @foreach ([['Conservative', '8–12', 'bg-blue-50 border-blue-200 text-blue-700'], ['Tolerant', '13–20', 'bg-green-50 border-green-200 text-green-700'], ['Moderate', '21–28', 'bg-amber-50 border-amber-200 text-amber-700'], ['Risk Taker', '29–32', 'bg-red-50 border-red-200 text-red-700']] as [$label, $range, $cls])
                                <div
                                    class="border rounded-lg px-3 py-2 {{ $cls }} {{ $result && $result->profile === $label ? 'ring-2 ring-offset-1 ring-current' : '' }}">
    ...
    @else
        ...
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="page-title">Kuesioner Profil Investasi</h1>
                <p class="page-sub">Jawab semua pertanyaan berikut untuk mengetahui profil investasi Anda</p>
            </div>
            <a href="{{ route('quiz.index') }}"
                class="text-sm text-muted hover:text-primary transition flex items-center gap-1">
            ...
            <form method="POST" action="{{ route('quiz.submit') }}">
                ...
                <div class="space-y-5">
                    @foreach ($questions as $q)
                        <div
                            class="bg-white rounded-2xl border border-line shadow-sm p-6 @error("answers.$q->id") border-red-300 @enderror">
                            <div class="flex items-start gap-3 mb-4">
                                <div
                                    class="w-8 h-8 rounded-lg bg-primary/10 text-primary font-bold text-sm grid place-items-center shrink-0">
                                    {{ $q->order }}</div>
                            ...
                            <div class="space-y-2 ml-11">
                                @foreach ($q->options as $opt)
                                    <label
                                        class="flex items-start gap-3 p-3 rounded-xl border border-line cursor-pointer hover:border-accent/40 hover:bg-accent/5 transition has-[:checked]:border-accent has-[:checked]:bg-accent/5">
                                        <input type="radio" name="answers[{{ $q->id }}]"
                                            value="{{ $opt->id }}" class="mt-0.5 accent-accent shrink-0"
