    ...
    <div x-data="{
        ...
        <div x-show="tab === 'detail'" x-cloak x-data="{
                    ...
                    <div class="bg-white rounded-2xl border border-line shadow-sm overflow-hidden mt-6">
                        <div class="px-6 py-4 border-b border-line bg-gradient-to-r from-primary to-primary-light">
                            <h2 class="font-bold text-white text-sm">{{ $section['label'] }}
                                @if ($selectedYear)
                                    <span class="text-[10px] font-normal opacity-75 ml-2">(Prospektus
                                        {{ $selectedYear }})</span>
                                @endif
                            </h2>
                        </div>
                            ...
                            <table class="w-full text-sm">
                                <thead>
                                    <tr class="bg-[#f8fafc] text-left text-muted text-xs uppercase tracking-wide">
                                        <th class="px-5 py-3 font-semibold">Nama / Pihak</th>
                                        <th class="px-5 py-3 font-semibold">Jabatan / Peran</th>
                                    </tr>
                                </thead>
                                    ...
                                    @foreach ($section['items'] as $item)
                                        <tr class="hover:bg-[#f8fafc] transition-colors">
                                            <td class="px-5 py-3 text-xs font-semibold">
                                                <button type="button" @click="openPerson({{ Js::from($item['name']) }})"
                                                    class="text-accent hover:underline text-left">
                                                    {{ $item['name'] }}
                                                </button>
                                            </td>
                                            <td class="px-5 py-3 text-xs text-muted">{{ $item['position'] ?: '-' }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                            ...
                            <tbody class="divide-y divide-line">
                                @foreach ($dpsItems as $item)
                                    <tr class="hover:bg-[#f8fafc] transition-colors">
                                        <td class="px-5 py-3 text-xs font-semibold">
                                            <button type="button" @click="openPerson({{ Js::from($item['name']) }})"
                                                class="text-accent hover:underline text-left">{{ $item['name'] }}</button>
                                        </td>
                                        <td class="px-5 py-3 text-xs text-muted">{{ $item['position'] ?: '-' }}</td>
                                    </tr>
                                @endforeach
            ...
            @if (!empty($pihakTerafiliasiItems))
                <div class="bg-white rounded-2xl border border-line shadow-sm overflow-hidden mt-6">
                    <div class="px-6 py-4 border-b border-line bg-gradient-to-r from-primary to-primary-light">
                        <h2 class="font-bold text-white text-sm">Pihak Terafiliasi</h2>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm">
                            <thead>
                            ...
                            <tbody class="divide-y divide-line">
                                @foreach ($pihakTerafiliasiItems as $line)
                                    ...
                                    @if (!empty($line))
                                        ...
                                        <tr class="hover:bg-[#f8fafc] transition-colors">
                                            <td class="px-5 py-3 text-xs font-semibold">{{ $parts[0] ?? $line }}</td>
                                            <td class="px-5 py-3 text-xs text-muted">{{ $parts[1] ?? '-' }}</td>
                                        </tr>
                                    @endif
                                @endforeach
                            </tbody>
        ...
        @if (!empty($pasardanaGovernance['commissioners']))
            <div class="bg-white rounded-2xl border border-line shadow-sm overflow-hidden mt-6 mb-3">
                <div class="px-6 py-4 border-b border-line bg-gradient-to-r from-primary to-primary-light">
        ...
        @if (!empty($pasardanaGovernance['shareholders']))
            <div x-show="tab === 'detail'" x-cloak
                class="bg-white rounded-2xl border border-line shadow-sm overflow-hidden mt-6">
                <div class="px-6 py-4 border-b border-line bg-gradient-to-r from-primary to-primary-light">
                ...
                @else
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="bg-[#f8fafc] text-left text-muted text-xs uppercase tracking-wide">
                                    <th class="px-5 py-3 font-semibold">Kode Reksa Dana</th>
                                    <th class="px-5 py-3 font-semibold">Nama Reksa Dana</th>
                                    <th class="px-5 py-3 font-semibold">Jenis</th>
                                    <th class="px-5 py-3 font-semibold">Kategori</th>
                                    <th class="px-5 py-3 font-semibold text-right">NAB/UP</th>
                                    <th class="px-5 py-3 font-semibold text-right">AUM</th>
                                    <th class="px-5 py-3 font-semibold">Tanggal Data</th>
                                    <th class="px-5 py-3 font-semibold text-right w-20">Aksi</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-line">
                                @foreach ($manager->funds as $fund)
                                    <tr class="hover:bg-[#f8fafc] transition-colors">
                                        <td class="px-5 py-3 text-xs font-mono">{{ $fund->kode_reksa_dana ?? '-' }}</td>
                                        <td class="px-5 py-3 text-xs font-semibold">{{ $fund->nama_reksa_dana ?? '-' }}
                                        </td>
                                        <td class="px-5 py-3 text-xs">{{ $fund->jenis ?? '-' }}</td>
                                        <td class="px-5 py-3 text-xs">{{ $fund->kategori_label ?? '-' }}</td>
                                        <td class="px-5 py-3 text-xs text-right tabular-nums">
                                            {{ $fund->nab_per_unit ? number_format($fund->nab_per_unit, 4, ',', '.') : '-' }}
                                        </td>
                                        <td class="px-5 py-3 text-xs text-right tabular-nums text-primary font-semibold">
                ...
                <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        if (document.getElementById('chartAum')) {
                            new ApexCharts(document.getElementById('chartAum'), options(chartData.aum.series, formatRupiah,
                                'aum-bulanan-manajer-investasi')).render();
                        }
                        if (document.getElementById('chartUp')) {
                            new ApexCharts(document.getElementById('chartUp'), options(chartData.up.series, formatUnit,
                                'total-up-bulanan-manajer-investasi')).render();
                        }
                    });
                </script>
            @endif
                ...
                Alpine.data('pdfProspektusTab', (partitionsByDoc, csrfToken, extractUrl) => ({
                    ...
                    async showPageContent(docId, pageId) {
                        ...
                        try {
                            const res = await fetch(
                                `/admin/daftar-reksa-dana/documents/${docId}/parsed-pages`, {
                                    headers: {
                            ...
                            if (page) {
                                this.selectedPageContent = '<pre class="text-xs whitespace-pre-wrap">' +
                                    this.escapeHtml(page.text_content) + '</pre>';
                                this.pageContentCache[cacheKey] = this.selectedPageContent;
                            }
                        } catch (e) {
                            this.selectedPageContent = '<span class="text-red-600">' + this.escapeHtml(e
                                .message) + '</span>';
                        }
                    },
                    ...
                    async savePartition() {
                        ...
                        if (parseInt(pm.start) > parseInt(pm.end)) {
                            pm.error =
                                'Halaman mulai harus lebih kecil atau sama dengan halaman selesai.';
                            return;
                        }
                    ...
                    async deletePartition(partitionId) {
                        ...
                        try {
                            const res = await fetch(
                                `/admin/daftar-reksa-dana/partitions/${partitionId}`, {
                                    method: 'DELETE',
                                    headers: {
                                        'Accept': 'application/json',
                                        'X-Requested-With': 'XMLHttpRequest',
                    ...
                    async parseToManager(docId, tahun) {
                        ...
                        try {
                            ...
                            this.selectedPartitionIds.forEach(pid => formData.append('partition_ids[]',
                                pid));
        ...
        <div x-show="tab === 'pdf-prospektus'" x-cloak x-data="pdfProspektusTab({{ Js::from($partitionsByDoc) }}, '{{ csrf_token() }}', '{{ route('admin.investment-managers.extract-prospektus-data', $manager) }}')">
            <div class="space-y-6">
                @forelse($managerFundsWithProspektus as $fund)
                        ...
                        <div class="bg-white rounded-2xl border border-line shadow-sm overflow-hidden">
                            <div
                                ...
                                <h2 class="font-bold text-white text-sm">
                                    ...
                                    @if ($doc->parsedPages->isNotEmpty())
                                        <span
                                            class="text-[10px] font-normal opacity-75 ml-2">({{ $doc->parsedPages->count() }}
                                            hlm diparse)</span>
                                    @endif
                                </h2>
                                <div class="flex items-center gap-2">
                                    <a target="_blank" href="{{ route('admin.daftar-reksa-dana.documents.view', $doc) }}"
                                        class="px-3 py-1.5 bg-white/20 hover:bg-white/30 text-white rounded-lg text-xs font-semibold transition">Preview
                                        PDF</a>
                            ...
                            @if ($doc->parsedPages->isEmpty())
                                <div class="p-6 text-center text-muted">
                                    ...
                                    <p class="text-xs mt-1">Upload dan parse dokumen ini di <a
                                            href="{{ route('admin.daftar-reksa-dana.index', ['tab' => 'prospektus-ffs']) }}"
                                            class="text-accent hover:underline">Daftar Reksa Dana</a>.</p>
                                </div>
                            @else
                                <div
                                    class="grid grid-cols-1 lg:grid-cols-4 gap-0 lg:gap-0 divide-y lg:divide-y-0 lg:divide-x divide-line">
                                    {{-- Kolom Partisi --}}
                                    <div class="p-4 lg:col-span-1">
                                        <div class="flex items-center justify-between mb-3">
                                            <h3 class="font-bold text-primary text-xs">Partisi</h3>
                                            <button @click="openPartitionModal({{ $doc->id }})"
                                                class="px-2 py-1 bg-primary text-white rounded text-[10px] font-semibold hover:bg-primary/90 transition">+
                                                Partisi</button>
                                        </div>
                                        <div class="space-y-1.5">
                                            @forelse($doc->partitions as $partition)
                                                <label
                                                    class="flex items-start gap-2 px-3 py-2 rounded-lg border border-line bg-[#f8fafc] hover:border-accent/30 transition cursor-pointer"
                                                    :class="selectedPartitionIds.includes({{ $partition->id }}) ?
                                                        'border-accent bg-accent/5' : ''">
                                                    <input type="checkbox" :value="{{ $partition->id }}"
                                                        x-model="selectedPartitionIds"
                                                        class="mt-0.5 rounded border-gray-300 text-accent focus:ring-accent/30 shrink-0">
                                                    <div class="min-w-0 flex-1">
                                                        <div class="flex items-center gap-1.5">
                                                            <p class="text-xs font-semibold text-primary truncate">
                                                                {{ $partition->nama_partisi }}</p>
                                                            @if ($partition->source === 'toc_ai')
                                                                <span
                                                                    class="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-medium bg-violet-50 text-violet-700 border border-violet-100">AI</span>
                                                            @endif
                                                        </div>
                                                        <p class="text-[10px] text-muted">
                                                            Parse {{ $partition->start_page }}-{{ $partition->end_page }}
                                                            @if ($partition->start_page_pdf && $partition->end_page_pdf)
                                                                <span class="text-[9px] text-slate-400">(PDF
                                                                    {{ $partition->start_page_pdf }}-{{ $partition->end_page_pdf }})</span>
                                                            @endif
                                                        </p>
                                                    </div>
                                                    <div class="flex items-center gap-0.5 shrink-0">
                                                        <button type="button"
                                                            @click.stop.prevent="openPartitionModal({{ $doc->id }}, {id: {{ $partition->id }}, nama_partisi: '{{ addslashes($partition->nama_partisi) }}', start_page: {{ $partition->start_page }}, end_page: {{ $partition->end_page }}})"
                                                            class="p-1 text-blue-400 hover:text-blue-600 rounded transition">
                                                        ...
                                                        <button type="button"
                                                            @click.stop.prevent="deletePartition({{ $partition->id }})"
                                            ...
                                            @empty
                                                <p class="text-xs text-muted italic">Belum ada partisi. Aktifkan "Otomatis
                                                    buat partisi dari daftar isi" saat parse, atau buat manual.</p>
                                            @endforelse
                                        </div>
                                        ...
                                        <div class="mt-4 pt-4 border-t border-line">
                                            <button
                                                @click='parseToManager({{ $doc->id }}, {{ $doc->ffs_year ?? 'null' }})'
                                                ...
                                                <span x-show="loading"
                                                    class="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin"></span>
                                                <span
                                                    x-text="loading ? 'Memproses...' : 'Parse ke Manajer Investasi'"></span>
                                            </button>
                                            <p class="text-[10px] text-muted mt-1">Pilih satu atau beberapa partisi, lalu
                                                klik tombol untuk mengekstrak data Manajer Investasi.</p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="p-4 lg:col-span-1">
                                        ...
                                        <p class="text-[10px] text-muted mb-2">Gunakan nomor <strong>Parsing</strong> saat
                                            membuat partisi.</p>
                                        <div class="space-y-1 max-h-96 overflow-y-auto">
                                            @foreach ($doc->parsedPages as $page)
                                                <div @click='showPageContent({{ $doc->id }}, {{ $page->id }})'
                                                    :class="isPageInSelectedPartition({{ $doc->id }},
                                                            {{ $page->page_parse }}) ?
                                                        'bg-accent/10 border border-accent/30' :
                                                        'border border-line hover:border-accent/30 hover:bg-[#f8fafc]'"
                                                    class="px-3 py-2 rounded-lg cursor-pointer transition text-xs flex items-center gap-2">
                                                    <span class="text-[10px] text-muted font-mono w-10 shrink-0">PDF
                                                        {{ $page->page_pdf }}</span>
                                                    <span class="text-[10px] text-muted">→ Parsing
                                                        {{ $page->page_parse }}</span>
                                                </div>
                                            @endforeach
                                        </div>
                                    </div>
                                    ...
                                    <div class="p-4 lg:col-span-2">
                                        ...
                                        <div x-show="!selectedPageContent"
                                            class="text-xs text-muted italic py-8 text-center">
                    ...
                    @empty
                        <div class="py-12 text-center text-muted bg-white rounded-2xl border border-line">
                            ...
                            <p class="text-xs mt-1">Upload di <a
                                    href="{{ route('admin.daftar-reksa-dana.index', ['tab' => 'prospektus-ffs']) }}"
                                    class="text-accent hover:underline">Daftar Reksa Dana</a>.</p>
                        </div>
                    @endforelse
                                    ...
                                    @foreach ($fields as $key => $label)
                                        ...
                                        @if (filled($val))
                                            <div class="px-6 py-2.5 flex items-start gap-4">
                                                <span
                                                    class="font-semibold text-muted w-36 shrink-0">{{ $label }}</span>
                ...
                <div x-show="partitionModal.open" x-cloak
                    class="fixed inset-0 z-50 flex items-center justify-center px-4 py-6"
                    @click.self="partitionModal.open = false">
                    <div class="bg-white rounded-2xl shadow-xl border border-line w-full max-w-md">
                        <div class="flex items-center justify-between px-6 py-4 border-b border-line">
                            <h3 class="font-bold text-primary"
                                x-text="partitionModal.editing ? 'Edit Partisi' : 'Tambah Partisi'"></h3>
                        ...
                        <div class="p-6 space-y-4">
                            ...
                            <p class="text-[11px] text-muted">Isi nomor halaman berdasarkan kolom <strong>Parsing</strong> di
                                daftar sebelah kiri, bukan nomor PDF asli.</p>
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-xs font-semibold text-primary mb-1">Halaman Parsing Mulai
                                        *</label>
                                    <input type="number" x-model="partitionModal.start" min="1" required
                                        class="w-full border border-line rounded-lg px-3 py-2 text-sm focus:border-accent focus:ring focus:ring-accent/30">
                                </div>
                                <div>
                                    <label class="block text-xs font-semibold text-primary mb-1">Halaman Parsing Selesai
                                        *</label>
                                    <input type="number" x-model="partitionModal.end" min="1" required
                                        class="w-full border border-line rounded-lg px-3 py-2 text-sm focus:border-accent focus:ring focus:ring-accent/30">
                                </div>
                            </div>
                            <div x-show="partitionModal.error" x-text="partitionModal.error"
                                class="px-3 py-2 rounded-lg text-xs bg-red-50 border border-red-200 text-red-700"></div>
                            <div class="flex justify-end gap-2 pt-2">
                                <button @click="partitionModal.open = false"
                                ...
                                <button @click="savePartition()" :disabled="partitionModal.saving"
                                    ...
                                    <span x-show="partitionModal.saving"
                                        class="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin inline-block mr-1 align-middle"></span>
                                    <span>Simpan</span>
                                </button>
                            </div>
                        </div>
                                ...
                                <div>
                                    ...
                                    <div class="overflow-x-auto" x-show="personModal.data.funds.length > 0">
                                        <table class="w-full text-sm">
                                            ...
                                            <tbody class="divide-y divide-line">
                                                <template x-for="row in personModal.data.funds"
                                ...
                                <div>
                                    ...
                                    <div class="overflow-x-auto" x-show="personModal.data.managers.length > 0">
                                        <table class="w-full text-sm">
                                            ...
                                            <tbody class="divide-y divide-line">
                                                <template x-for="row in personModal.data.managers"
                                                    :key="row.name + row.role + row.position">
                                                    <tr>
                                                        <td class="px-3 py-2 font-semibold" x-text="row.name"></td>
                                                        <td class="px-3 py-2" x-text="row.role || '-'"></td>
