...
@section('content')
    <div x-data="{
        ...
        <div class="mb-6 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                <form method="POST" action="{{ route('admin.daftar-reksa-dana.sync-pasardana') }}"
                    @submit="submitSync($event)" data-poll-url="{{ url('admin/daftar-reksa-dana/sync-pasardana/status') }}">
                    @csrf
                ...
                <form method="POST" action="{{ route('admin.daftar-reksa-dana.replace-rewrite') }}"
                    @submit="submitSync($event)" data-poll-url="{{ url('admin/daftar-reksa-dana/sync-pasardana/status') }}">
                    @csrf
                ...
                <form method="POST" action="{{ route('admin.daftar-reksa-dana.sync-all-pasardana') }}"
        ...
        @if ($tab === 'harga')
            ...
            <div class="table-card">
                ...
                @if ($lastSyncRun)
                    <div
                        class="px-5 py-2 bg-blue-50 border-b border-blue-200 flex items-center gap-2 text-xs text-blue-800">
                ...
                {{-- Import Excel --}}
                <div class="px-6 py-3 border-b border-line bg-blue-50/50">
                    <form method="POST" action="{{ route('admin.daftar-reksa-dana.import-excel') }}"
                        enctype="multipart/form-data" class="flex flex-wrap items-center gap-3">
                        @csrf
                        <input type="hidden" name="tab" value="harga">
                        <span class="text-xs font-semibold text-muted">Import Harga via Excel:</span>
                        <input type="file" name="file" accept=".xlsx,.xls,.csv" required
                            class="flex-1 min-w-[160px] text-xs border border-line rounded-lg px-2 py-1.5 file:mr-1 file:py-0.5 file:px-2 file:rounded file:border-0 file:text-[10px] file:bg-black file:text-white">
                        <button type="submit"
                            class="px-3 py-1.5 bg-black text-white rounded-lg text-xs font-semibold hover:bg-blue-700 whitespace-nowrap">Upload</button>
                    </form>
                </div>
                                    ...
                                    <a href="{{ route('admin.daftar-reksa-dana.index', array_merge(request()->except('sort', 'direction', 'harga_page'), ['tab' => 'harga', 'sort' => 'nama_reksa_dana', 'direction' => request('sort', 'nama_reksa_dana') === 'nama_reksa_dana' && request('direction', 'asc') === 'asc' ? 'desc' : 'asc'])) }}"
                                        ...
                                        @if (request('sort', 'nama_reksa_dana') === 'nama_reksa_dana')
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="{{ request('direction', 'asc') === 'asc' ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7' }}" />
                                            </svg>
                                        @endif
                                    </a>
                                </th>
        ...
        @elseif($tab === 'harian')
            ...
            <div class="table-card">
                ...
                {{-- Import Excel --}}
                <div class="px-6 py-3 border-b border-line bg-orange-50/50">
                    <form method="POST" action="{{ route('admin.daftar-reksa-dana.import-excel') }}"
                        enctype="multipart/form-data" class="flex flex-wrap items-center gap-3">
                        @csrf
                        <input type="hidden" name="tab" value="harian">
                        <span class="text-xs font-semibold text-muted">Import Harian via Excel:</span>
                        <input type="file" name="file" accept=".xlsx,.xls,.csv" required
                            class="flex-1 min-w-[160px] text-xs border border-line rounded-lg px-2 py-1.5 file:mr-1 file:py-0.5 file:px-2 file:rounded file:border-0 file:text-[10px] file:bg-slate-700 file:text-white">
                        <button type="submit"
                            class="px-3 py-1.5 bg-slate-700 text-white rounded-lg text-xs font-semibold hover:bg-slate-900 whitespace-nowrap">Upload</button>
                    </form>
                </div>

                @if ($lastSyncRun)
                    <div
                        class="px-5 py-2 bg-orange-50 border-b border-orange-200 flex items-center gap-2 text-xs text-orange-800">
                        <svg class="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                        </svg>
                    ...
                    <table class="w-full text-sm">
                                    ...
                                    <a href="{{ route('admin.daftar-reksa-dana.index', array_merge(request()->except('harian_sort', 'harian_direction', 'harian_page'), ['tab' => 'harian', 'harian_sort' => 'nama_reksa_dana', 'harian_direction' => request('harian_sort', 'nama_reksa_dana') === 'nama_reksa_dana' && request('harian_direction', 'asc') === 'asc' ? 'desc' : 'asc'])) }}"
                                        ...
                                        @if (request('harian_sort', 'nama_reksa_dana') === 'nama_reksa_dana')
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="{{ request('harian_direction', 'asc') === 'asc' ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7' }}" />
                                            </svg>
                                        @endif
                                    </a>
                                </th>
                        ...
                        <tbody class="divide-y divide-line">
                            @forelse($harian as $h)
                                <tr class="hover:bg-[#f8fafc] transition-colors">
                                    <td class="px-4 py-3.5 text-xs text-muted">
                                        {{ $h->tanggal_nab ? $h->tanggal_nab->format('d M Y') : '—' }}</td>
                            ...
                            <table class="w-full text-sm">
                                ...
                                <tbody class="divide-y divide-line">
                                    @foreach ($recentSyncRuns as $run)
                                        ...
                                        @endphp
                                        <tr
                                            class="hover:bg-[#f8fafc] transition-colors {{ $isSelected ? 'bg-primary/5' : '' }}">
                                            <td class="px-4 py-3 font-mono text-xs">#{{ $run->id }}</td>
                                            <td class="px-4 py-3 text-xs text-muted">
                                                {{ $run->created_at->format('d M Y H:i') }}</td>
                                            <td class="px-4 py-3">
                                                <span
                                                    class="inline-flex px-2 py-0.5 rounded border text-xs font-semibold {{ $statusClass }}">
                                                    {{ $run->status }}
                                                </span>
                                            </td>
                                            <td class="px-4 py-3 text-xs">{{ $run->progress_percent }}%</td>
                                            <td class="px-4 py-3 text-xs text-muted max-w-[200px] truncate">
                                                {{ $run->message ?: '—' }}</td>
                                            <td class="px-4 py-3">
                ...
                <div class="table-card" x-data="{ syncDetailTab: 'changes' }">
                    <div class="table-head">
                        ...
                        @if ($selectedRun && $selectedRun->applied_at)
                            <span class="text-xs text-white/70 font-normal">✓ Applied
                                {{ $selectedRun->applied_at->format('d M Y H:i') }}</span>
                        @endif
                    </div>
                    ...
                    @if ($selectedRun && $changesUrl)
                        {{-- Sub-tabs --}}
                        @php
                            $existingNames = \App\Models\ReksaDana::pluck('nama_reksa_dana')
                        ...
                        @if ($pendingRdList->isNotEmpty())
                            <div class="flex gap-1 px-4 pt-3 border-b border-line">
                                <button @click="syncDetailTab = 'changes'"
                                    ...
                                    :class="syncDetailTab === 'changes' ? 'border-primary text-primary' :
                                        'border-transparent text-muted hover:text-primary'">
                                    Perubahan
                                </button>
                                <button @click="syncDetailTab = 'pending'"
                                    class="px-4 py-2 text-xs font-semibold border-b-2 -mb-px transition"
                                    :class="syncDetailTab === 'pending' ? 'border-emerald-600 text-emerald-600' :
                        ...
                        @if ($pendingRdList->isNotEmpty())
                            <div x-show="syncDetailTab === 'pending'" x-cloak class="p-4">
                                <form method="POST"
                                    action="{{ route('admin.daftar-reksa-dana.sync-pasardana.apply', $selectedRun) }}"
                                            ...
                                            <thead>
                                                <tr
                                                    ...
                                                    <th class="px-4 py-3 font-semibold w-10">
                                                        <input type="checkbox" id="check-all-rd-baru"
                                            ...
                                            <tbody class="divide-y divide-line">
                                                @foreach ($pendingRdList as $i => $prd)
                                                    ...
                                                    <tr class="hover:bg-[#f8fafc] transition-colors">
                                                        ...
                                                        <td class="px-4 py-2.5 text-xs text-muted">{{ $i + 1 }}
                                                        </td>
                                                        <td class="px-4 py-2.5 text-xs font-medium text-primary">
                                                            {{ $pData['nama_reksa_dana'] ?? '-' }}</td>
                                                        <td class="px-4 py-2.5 text-xs text-muted">
                                                            {{ $pData['jenis'] ?? '-' }}</td>
                                                        <td class="px-4 py-2.5 text-xs text-muted max-w-[150px] truncate">
                                                            {{ $pData['nama_manajer_investasi'] ?? '-' }}</td>
                                                        <td class="px-4 py-2.5 text-xs font-mono">
                                                            {{ isset($pData['nab_per_unit']) ? number_format($pData['nab_per_unit'], 2) : '-' }}
                                                        </td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
                                    ...
                                    @if (!$selectedRun->applied_at)
                                        <div class="mt-4 flex items-center justify-between gap-3">
                                            <p class="text-xs text-amber-600">
                                                <svg class="w-3.5 h-3.5 inline -mt-0.5" fill="none"
                                    ...
                                    @else
                                        <p class="mt-3 text-xs text-emerald-600">✓ Semua RD baru sudah diterapkan pada
                                            {{ $selectedRun->applied_at->format('d M Y H:i') }}</p>
                                    @endif
                                </form>
                            </div>
                        @endif
        ...
        <script>
            ...
            function openEditModalFromData(doc) {
                ...
                document.getElementById('form-document-edit').action =
                    '{{ route('admin.daftar-reksa-dana.documents.update', '_docid_') }}'.replace('_docid_', doc.id);
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.addEventListener('click', function(e) {
                    ...
                    if (editBtn) {
                        ...
                        document.getElementById('form-document-edit').action =
                            '{{ route('admin.daftar-reksa-dana.documents.update', '_docid_') }}'.replace(
                                '_docid_', docId);
                        document.getElementById('edit-doc-notes').value = docNotes || '';
                        const typeSelect = document.querySelector(
                            '#form-document-edit select[name="document_type"]');
                    ...
                    try {
                        ...
                        if (partitionInfoEl) {
                            if (json.data.partitions_created > 0) {
                                partitionInfoEl.textContent = json.data.partitions_created +
                                    ' partisi dibuat dari daftar isi.';
                ...
                function openEditHarga(data) {
                    ...
                    form.action = '{{ route('admin.daftar-reksa-dana.harga.update', 'REPLACE_ID') }}'.replace(
                        'REPLACE_ID', data
                        .id);
                    ...
                    const kategori = Array.isArray(data.kategori) ? data.kategori : [];
                    document.getElementById('edit-harga-kategori-display').textContent = kategori.length ? kategori
                        .join(', ') :
                        '—';
        ...
        <div x-show="isSyncing" x-cloak
            ...
            <div class="text-center max-w-lg w-full">
                <template x-if="syncStatus !== 'completed' && syncStatus !== 'failed'">
                    <div
                        class="w-14 h-14 border-4 border-accent border-t-transparent rounded-full mx-auto mb-6 animate-spin">
                    </div>
                </template>
                ...
                <h3 class="text-lg font-bold text-primary mb-1">
                    ...
                    <span x-show="syncStatus !== 'completed' && syncStatus !== 'failed'">Sinkronisasi RD dari
                        Pasardana</span>
                </h3>
                <p class="text-sm text-muted mb-2" x-text="syncStepLabel"></p>
                <p class="text-xs text-muted mb-5">
                    Run ID: <span class="font-mono" x-text="syncRunId"></span> &middot; Status: <span
                        class="font-semibold" x-text="syncStatus"></span>
                </p>

                <div class="max-w-sm mx-auto mb-5">
                    <div class="h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div class="h-full transition-all duration-500 ease-out" :style="`width: ${syncProgress}%`"
                            :class="syncStatus === 'failed' ? 'bg-red-500' : (syncStatus === 'completed' ? 'bg-green-500' :
                                'bg-accent')">
                        </div>
                    </div>
                ...
                <template x-if="syncMessage">
                    <div class="mt-2 mx-auto max-w-md text-left rounded-xl border px-4 py-3 text-xs"
                        :class="syncStatus === 'failed' ? 'border-red-200 bg-red-50 text-red-800' :
                ...
                <template x-if="syncErrors && syncErrors.length">
                    <div
                        class="mt-3 mx-auto max-w-md text-left rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-800">
    ...
    {{-- Modal Parse Dokumen --}}
    <div id="modal-document-parse" class="fixed inset-0 z-50 hidden bg-black/40 flex items-center justify-center p-4"
        onclick="if(event.target===this)closeModal('modal-document-parse')">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto"
            onclick="event.stopPropagation()">
            <div class="flex items-center justify-between px-6 py-4 border-b border-line sticky top-0 bg-white z-10">
                <div>
                    <h3 class="font-bold text-primary">Parse Dokumen</h3>
                    <p class="text-xs text-muted mt-0.5">Nama file: <span id="parse-doc-filename"
                            class="font-semibold text-primary">—</span></p>
                </div>
                <button type="button" onclick="closeModal('modal-document-parse')"
                    class="p-1 hover:bg-[#f1f5f9] rounded-lg transition">
                    <svg class="w-5 h-5 text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="p-6 space-y-4">
                <p class="text-sm text-muted">Proses ini mengekstrak teks dari setiap halaman PDF setelah daftar isi. Hasil
                    disimpan ke database.</p>
                <div id="parse-loading-el" class="hidden text-xs text-muted">Menghitung total halaman...</div>
                <span id="parse-badge"
                    class="hidden inline-flex items-center gap-1 px-2 py-0.5 bg-emerald-50 text-emerald-700 rounded-full text-xs font-semibold">Sudah
                    diparse: <span id="parse-badge-count">0</span> halaman</span>
                <form id="form-document-parse" action="{{ route('admin.daftar-reksa-dana.documents.parse') }}"
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="number" name="toc_start_page" min="1" value="1" required
                                class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-primary mb-1">Halaman Daftar Isi Selesai
                                *</label>
                            <input type="number" name="toc_end_page" min="1" value="4" required
                                class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                        </div>
                        <div class="flex items-start gap-2 pt-1">
                            <input type="checkbox" name="generate_partitions" id="generate_partitions" value="1"
                                checked class="mt-0.5 w-4 h-4 text-emerald-700 border-line rounded focus:ring-emerald-700">
                            <label for="generate_partitions" class="text-sm text-primary cursor-pointer select-none">
                                ...
                                <span class="block text-xs text-muted font-normal">AI akan membaca daftar isi dan membuat
                                    partisi per bab secara otomatis.</span>
                            </label>
                        </div>
                    </div>
                    <div id="parse-error"
                        class="hidden mt-4 px-4 py-3 rounded-xl text-sm bg-red-50 border border-red-200 text-red-700">
                    </div>
                    <div id="parse-success"
                        class="hidden mt-4 px-4 py-3 rounded-xl text-sm bg-green-50 border border-green-200 text-green-700">
                    </div>
                    <div id="parse-result" class="hidden mt-4 bg-emerald-50 border border-emerald-200 rounded-xl p-4">
                        ...
                        <p class="text-xs text-muted mt-1"><span id="parse-result-count">0</span> halaman teks dari <span
                                id="parse-result-total">0</span> total halaman PDF.</p>
                        <p id="parse-result-partitions" class="text-xs text-emerald-700 font-medium mt-1 hidden"></p>
                        <div id="parse-result-warnings"
                            class="hidden mt-2 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                        </div>
                    </div>
                    <div class="flex justify-end gap-2 mt-6">
                        <button type="button" onclick="closeModal('modal-document-parse')"
                            class="px-4 py-2 text-sm text-muted border border-line rounded-lg hover:bg-[#f1f5f9] transition">Batal</button>
                        <button type="button" id="btn-submit-parse"
                            class="px-4 py-2 text-sm text-white bg-emerald-700 rounded-lg hover:bg-emerald-800 transition disabled:opacity-50"><span>Parse</span></button>
                    </div>
                </form>
            </div>
        </div>
