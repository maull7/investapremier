<div class="space-y-6">
                ...
                <div class="mt-4 max-w-sm mx-auto">
                    <div class="bg-[#f8fafc] border border-line rounded-xl p-4 text-left space-y-3">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            ...
                            <input type="number" x-model="uploadFfsYear" placeholder="Tahun" min="2000"
                                max="2100" class="w-full text-xs border border-line rounded-lg px-3 py-2">
                        </div>
                        <button @click="uploadFfs()"
                            :disabled="uploadFfsLoading || !uploadFfsFile || !uploadFfsMonth || !uploadFfsYear"
                        ...
                        <div x-show="uploadFfsSuccess" x-text="uploadFfsSuccess"
                            class="px-3 py-2 rounded-lg text-xs bg-green-50 border border-green-200 text-green-700">
                        </div>
                    </div>
                </div>
            @else
        ...
        <div class="bg-white rounded-2xl border border-line shadow-sm overflow-hidden">
            <div
                ...
                <h2 class="font-bold text-white text-sm">
                    ...
                    @if ($doc->parsedPages->isNotEmpty())
                        <span class="text-[10px] font-normal opacity-75 ml-2">({{ $doc->parsedPages->count() }} hlm
                            diparse)</span>
                    @endif
                </h2>
            ...
            @if ($doc->parsedPages->isEmpty())
                <div class="p-6 text-center text-muted">
                    ...
                    <p class="text-xs mt-1">Gunakan tombol <strong>Parse Dokumen</strong> di tab Prospektus dan FFS
                        untuk mengekstrak teks.</p>
                </div>
            @else
                <div
                    ...
                    <div class="p-4 lg:col-span-1">
                        @if ($docType === 'ffs')
                            ...
                            <div class="space-y-3">
                                ...
                                <div x-show="uploadFfsOpen"
                                    class="bg-[#f8fafc] border border-line rounded-xl p-4 space-y-3">
                                    <p class="text-xs font-semibold text-primary">Upload FFS Baru</p>
                                    ...
                                    <div x-show="uploadFfsError" x-text="uploadFfsError"
                                        class="px-3 py-2 rounded-lg text-xs bg-red-50 border border-red-200 text-red-700">
                                    </div>
                                    <div x-show="uploadFfsSuccess" x-text="uploadFfsSuccess"
                                        class="px-3 py-2 rounded-lg text-xs bg-green-50 border border-green-200 text-green-700">
                                    </div>
                                ...
                                <div>
                                    ...
                                    <p class="text-[10px] text-muted">Untuk dokumen FFS, data langsung diparse dari
                                        seluruh halaman tanpa perlu partisi.</p>
                                </div>
                                ...
                                @if ($latestFfs)
                                    <div class="px-3 py-2 bg-emerald-50 border border-emerald-200 rounded-lg">
                                        ...
                                        <p class="text-[10px] text-muted mt-0.5">
                                            Periode:
                                            {{ $latestFfs->ffs_month ? str_pad($latestFfs->ffs_month, 2, '0', STR_PAD_LEFT) : '-' }}/{{ $latestFfs->ffs_year ?: '-' }}
                                        </p>
                                ...
                                <button @click='handleParseFfs({{ $doc->id }})'
                                    ...
                                    class="w-full px-3 py-2 bg-emerald-700 text-white rounded-lg text-xs font-semibold hover:bg-emerald-800 transition disabled:opacity-50 flex items-center justify-center gap-2">
                                    <span x-show="loadingFfs[{{ $doc->id }}]"
                                        class="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin"></span>
                                    <span
                                        x-text="loadingFfs[{{ $doc->id }}] ? 'Memproses...' : 'Parse FFS & Simpan'"></span>
                                </button>

                                <div x-show="ffsSuccess[{{ $doc->id }}]"
                                    x-text="ffsSuccess[{{ $doc->id }}]"
                                    class="px-3 py-2 rounded-lg text-xs bg-green-50 border border-green-200 text-green-700">
                                </div>
                        ...
                        @else
                            ...
                            <div class="space-y-1.5">
                                @forelse($doc->partitions as $partition)
                                    <label
                                        ...
                                        <button type="button"
                                            ...
                                            class="p-1 text-red-400 hover:text-red-600 rounded transition shrink-0">
                                ...
                                @empty
                                    <p class="text-xs text-muted italic">Belum ada partisi. Aktifkan "Otomatis buat
                                        partisi dari daftar isi" saat parse, atau buat manual.</p>
                                @endforelse
                            </div>
                            ...
                            <div class="mt-4 pt-4 border-t border-line">
                                ...
                                <p class="text-[10px] text-muted mt-1">Pilih satu atau beberapa partisi, lalu klik
                                    tombol untuk mengekstrak data dengan AI.</p>
                                <div x-show="error" x-text="error"
                                    class="mt-3 px-3 py-2 rounded-lg text-xs bg-red-50 border border-red-200 text-red-700">
                                </div>
                                <div x-show="success" x-text="success"
                                    class="mt-3 px-3 py-2 rounded-lg text-xs bg-green-50 border border-green-200 text-green-700">
                                </div>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="p-4 lg:col-span-1">
                        ...
                        <p class="text-[10px] text-muted mb-2">Gunakan nomor <strong>Parsing</strong> (bukan PDF asli)
                            saat membuat partisi.</p>
    ...
    <div x-show="partitionModal.open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4 py-6"
