...
@section('content')
    <div x-data="reksaDanaShow()">
        <div class="mb-6">
            <div class="flex items-center gap-2 text-sm text-muted mb-3">
                <a href="{{ route('admin.daftar-reksa-dana.index') }}" class="hover:text-primary transition">Daftar Reksa
                    Dana</a>
            ...
            <form method="POST" action="{{ route('admin.daftar-reksa-dana.export-investment-manager', $fund) }}"
                class="mt-3">
                @csrf
                <button type="submit"
                    class="px-4 py-2 bg-primary text-white rounded-xl text-sm font-semibold hover:bg-primary/90 transition">
                    Export ke Data Manajer Investasi
                </button>
            </form>
            <div class="flex flex-wrap gap-3 mt-2 text-sm text-muted">
                @if ($fund->kode_reksa_dana)
                    <span class="font-mono text-xs bg-[#f1f5f9] px-2 py-1 rounded">{{ $fund->kode_reksa_dana }}</span>
                @endif
                @if ($fund->nama_manajer_investasi)
                    <span>{{ $fund->nama_manajer_investasi }}</span>
                @endif
                @if ($fund->jenis)
                    <span
                        class="px-2 py-0.5 rounded-full text-xs font-semibold bg-blue-100 text-blue-700">{{ $fund->jenis }}</span>
                @endif
                @if ($fund->risk_category)
                    <span
                        class="px-2 py-0.5 rounded-full text-xs font-semibold {{ $fund->risk_category == 'Rendah' ? 'bg-green-100 text-green-700' : ($fund->risk_category == 'Sedang' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700') }}">{{ $fund->risk_category }}</span>
                @endif
                @if ($fund->tanggal_nab)
                    <span>Data: {{ $fund->tanggal_nab->format('d M Y') }}</span>
                @endif
            </div>
            <div class="flex items-center gap-3 mt-3 flex-wrap">
                <span class="text-xs font-semibold text-muted">Data Portfolio:</span>
                <div class="flex items-center gap-2 flex-wrap">
                    <select x-model="portfolioMonth" @change="applyPeriodFilter()"
                        class="border border-line rounded-lg px-3 py-1.5 text-xs text-muted">
                    ...
                    <select x-model="portfolioYear" @change="applyPeriodFilter()"
                        class="border border-line rounded-lg px-3 py-1.5 text-xs text-muted">
        ...
        @if ($periodActive)
            <div
                class="mb-4 px-4 py-3 bg-accent/5 border border-accent/20 rounded-xl text-sm text-primary flex items-center gap-2 flex-wrap">
                ...
                <span>Menampilkan data periode <strong>{{ $monthNames[$periodMonth - 1] ?? '-' }}
                        {{ $periodYear }}</strong> di semua tab.</span>
                @unless ($periodHasSnapshotData)
                    <span class="text-xs text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full">Belum ada data tersimpan untuk
                        periode ini.</span>
                @endunless
            </div>
        @endif
        ...
        @php
            $formatAum = function ($aumVal) {
                ...
                if ($aumVal >= 1_000_000_000_000) {
                    return 'Rp' . number_format($aumVal / 1_000_000_000_000, 2, ',', '.') . 'T';
                }
                if ($aumVal >= 1_000_000_000) {
                    return 'Rp' . number_format($aumVal / 1_000_000_000, 1, ',', '.') . 'M';
                }
                if ($aumVal >= 1_000_000) {
                    return 'Rp' . number_format($aumVal / 1_000_000, 1, ',', '.') . 'jt';
                }
                return 'Rp' . number_format($aumVal, 0, ',', '.');
            };
            $unitPenyertaan =
                $displayAum !== null && $displayNab !== null && $displayNab > 0 ? $displayAum / $displayNab : null;
        ...
        <div class="grid grid-cols-2 md:grid-cols-6 gap-4 mb-6">
            <div class="bg-white rounded-xl border border-line p-4">
                ...
                <p class="text-sm font-bold text-primary">
                    {{ $displayNab !== null ? number_format($displayNab, 4, ',', '.') : '—' }}</p>
            </div>
            <div class="bg-white rounded-xl border border-line p-4">
                <p class="text-xs text-muted mb-1">Return Bulanan</p>
                <p
                    class="text-sm font-bold {{ $displayReturnMonthly !== null ? ($displayReturnMonthly >= 0 ? 'text-green-600' : 'text-red-600') : 'text-muted' }}">
                    {{ $displayReturnMonthly !== null ? number_format($displayReturnMonthly, 2, ',', '.') . '%' : '—' }}
                </p>
            </div>
            <div class="bg-white rounded-xl border border-line p-4">
                <p class="text-xs text-muted mb-1">Return YTD</p>
                <p
                    class="text-sm font-bold {{ $displayReturnYtd !== null ? ($displayReturnYtd >= 0 ? 'text-green-600' : 'text-red-600') : 'text-muted' }}">
                    {{ $displayReturnYtd !== null ? number_format($displayReturnYtd, 2, ',', '.') . '%' : '—' }}</p>
            </div>
            <div class="bg-white rounded-xl border border-line p-4">
                <p class="text-xs text-muted mb-1">Return Tahunan</p>
                <p
                    class="text-sm font-bold {{ $displayReturnYearly !== null ? ($displayReturnYearly >= 0 ? 'text-green-600' : 'text-red-600') : 'text-muted' }}">
                    {{ $displayReturnYearly !== null ? number_format($displayReturnYearly, 2, ',', '.') . '%' : '—' }}</p>
            </div>
            <div class="bg-white rounded-xl border border-line p-4">
            ...
            <div class="bg-white rounded-xl border border-line p-4">
                ...
                <p class="text-sm font-bold text-primary">
                    {{ $unitPenyertaan !== null ? number_format($unitPenyertaan, 0, ',', '.') : '—' }}</p>
            </div>
        </div>

        {{-- Tab Navigation --}}
        <div class="flex items-center gap-1 mb-6 border-b border-line overflow-x-auto">
            <button @click="setTab('snapshot')"
                :class="tab === 'snapshot' ? 'border-accent text-accent' : 'border-transparent text-muted hover:text-primary'"
                class="px-5 py-3 text-sm font-semibold border-b-2 transition whitespace-nowrap">Snapshot</button>
            <button @click="setTab('grafik')"
            ...
            <button @click="setTab('risiko')"
                :class="tab === 'risiko' ? 'border-accent text-accent' : 'border-transparent text-muted hover:text-primary'"
                class="px-5 py-3 text-sm font-semibold border-b-2 transition whitespace-nowrap">Risiko</button>
            <button @click="setTab('biaya')"
                :class="tab === 'biaya' ? 'border-accent text-accent' : 'border-transparent text-muted hover:text-primary'"
                class="px-5 py-3 text-sm font-semibold border-b-2 transition whitespace-nowrap">Biaya</button>
            <button @click="setTab('portofolio')"
                :class="tab === 'portofolio' ? 'border-accent text-accent' : 'border-transparent text-muted hover:text-primary'"
                class="px-5 py-3 text-sm font-semibold border-b-2 transition whitespace-nowrap">Portofolio</button>
            <button @click="setTab('pdf-prospektus')"
                :class="tab === 'pdf-prospektus' ? 'border-accent text-accent' :
                    'border-transparent text-muted hover:text-primary'"
                class="px-5 py-3 text-sm font-semibold border-b-2 transition whitespace-nowrap">PDF Prospektus</button>
            <button @click="setTab('pdf-ffs')"
                :class="tab === 'pdf-ffs' ? 'border-accent text-accent' : 'border-transparent text-muted hover:text-primary'"
                class="px-5 py-3 text-sm font-semibold border-b-2 transition whitespace-nowrap">PDF FFS</button>
            <button @click="setTab('ekstrak-dokumen')"
                :class="tab === 'ekstrak-dokumen' ? 'border-accent text-accent' :
                    'border-transparent text-muted hover:text-primary'"
                class="px-5 py-3 text-sm font-semibold border-b-2 transition whitespace-nowrap">Daftar Ekstrak
                Dokumen</button>
        </div>
        ...
        <div x-show="tab === 'snapshot'" x-cloak>
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-white rounded-2xl border border-line shadow-sm overflow-hidden">
                    <div
                        class="px-6 py-4 border-b border-line bg-gradient-to-r from-primary to-primary-light flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            <button @click="toggleLock('info')" :disabled="toggling"
                                ...
                                :class="isLocked('info') ? 'bg-amber-400/30 text-amber-100 hover:bg-amber-400/40' :
                                    'bg-white/20 text-white hover:bg-white/30'">
                                <template x-if="isLocked('info')"><svg class="w-3.5 h-3.5" fill="none"
                    ...
                    <div class="divide-y divide-line">
                        <div class="px-6 py-3.5 flex items-start gap-4"><span
                                class="text-xs font-semibold text-muted w-36 shrink-0">Nama Reksa Dana</span><span
                                class="text-sm">{{ $fund->nama_reksa_dana }}</span></div>
                        @if ($fund->kode_reksa_dana)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-36 shrink-0">Kode Reksa Dana</span><span
                                    class="text-sm font-mono">{{ $fund->kode_reksa_dana }}</span></div>
                        @endif
                        <div class="px-6 py-3.5 flex items-start gap-4"><span
                                class="text-xs font-semibold text-muted w-36 shrink-0">Kode ISIN</span><span
                                class="text-sm font-mono">{{ $fund->isin_code ?: '-' }}</span></div>
                        @if ($fund->investmentManager || $fund->nama_manajer_investasi)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-36 shrink-0">Manajer Investasi</span><span
                                    class="text-sm">
                                    @if ($fund->investmentManager)
                                        <a href="{{ route('admin.investment-managers.show', $fund->investmentManager) }}"
                                            class="text-accent hover:underline">{{ $fund->nama_manajer_investasi }}</a>@else{{ $fund->nama_manajer_investasi }}
                                    @endif
                                </span></div>
                        @endif
                        @if ($fund->custodian_bank)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-36 shrink-0">Bank Kustodian</span><span
                                    class="text-sm">{{ $fund->custodian_bank }}</span></div>
                        @endif
                        <div class="px-6 py-3.5 flex items-start gap-4"><span
                                class="text-xs font-semibold text-muted w-36 shrink-0">Tanggal Efektif</span><span
                                class="text-sm">{{ $fund->launch_date?->format('d M Y') ?: '-' }}</span></div>
                        @php $launchDate = $fund->launch_date; @endphp
                        <div class="px-6 py-3.5 flex items-start gap-4"><span
                                class="text-xs font-semibold text-muted w-36 shrink-0">Tanggal Peluncuran</span><span
                                class="text-sm">{{ $launchDate?->format('d M Y') ?: '-' }}</span></div>
                        @if ($fund->tujuan_investasi)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-36 shrink-0">Tujuan Investasi</span><span
                                    class="text-sm">{{ $fund->tujuan_investasi }}</span></div>
                        @endif
                        @if ($fund->kebijakan_investasi)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-36 shrink-0">Kebijakan Investasi</span><span
                                    class="text-sm">{{ $fund->kebijakan_investasi }}</span></div>
                        @endif
                        <div class="px-6 py-3.5 flex items-start gap-4"><span
                                class="text-xs font-semibold text-muted w-36 shrink-0">Benchmark Tolak Ukur</span><span
                                class="text-sm">{{ $fund->benchmark ?: '-' }}</span></div>
                        @if ($fund->display_mata_uang)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-36 shrink-0">Mata Uang</span><span
                                    class="text-sm">{{ $fund->display_mata_uang }}</span></div>
                        @endif
                        @if ($fund->jenis)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-36 shrink-0">Kategori</span><span
                                    class="text-sm">{{ $fund->kategori_label ?: $fund->jenis }}</span></div>
                        @endif
                        @if ($fund->jenis)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-36 shrink-0">Jenis Reksa Dana</span><span
                                    class="text-sm">{{ $fund->jenis }}</span></div>
                        @endif
                        @if ($fund->kategori_produk)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-36 shrink-0">Kategori Produk</span><span
                                    class="text-sm">{{ $fund->kategori_produk }}</span></div>
                        @endif
                        @if ($fund->display_kelas)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-36 shrink-0">Kelas</span><span
                                    class="text-sm">{{ $fund->display_kelas }}</span></div>
                        @endif
                        @if ($fund->is_etf !== null)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-36 shrink-0">ETF</span><span
                                    class="text-sm">{{ $fund->is_etf ? 'Ya' : 'Tidak' }}</span></div>
                        @endif
                        @if ($fund->is_index !== null)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-36 shrink-0">Index Fund</span><span
                                    class="text-sm">{{ $fund->is_index ? 'Ya' : 'Tidak' }}</span></div>
                        @endif
                        @if ($fund->conservative_category)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-36 shrink-0">Kategori Konservatif</span><span
                                    class="text-sm">{{ $fund->conservative_category }}</span></div>
                        @endif
                        @if ($fund->dividend !== null)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-36 shrink-0">Dividen</span><span
                                    class="text-sm">{{ $fund->dividend ? 'Ya' : 'Tidak' }}</span></div>
                        @endif
                        <div class="px-6 py-3.5 flex items-start gap-4"><span
                                class="text-xs font-semibold text-muted w-36 shrink-0">Maks. Unit Penyertaan</span><span
                                class="text-sm font-bold text-primary">{{ $maxUnit ? number_format($maxUnit, 0, ',', '.') : '—' }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-line shadow-sm overflow-hidden">
                    ...
                    <div class="divide-y divide-line">
                        <div class="px-6 py-3.5 flex items-start gap-4"><span
                                class="text-xs font-semibold text-muted w-36 shrink-0">NAV / NAB-UP</span><span
                                class="text-sm font-bold text-primary">{{ $displayNab !== null ? number_format($displayNab, 4, ',', '.') : '—' }}</span>
                        </div>
                        {{-- @if (!$periodActive)
                ...
                @endif --}}
                        <div class="px-6 py-3.5 flex items-start gap-4"><span
                                class="text-xs font-semibold text-muted w-36 shrink-0">Return 1 Bulan</span><span
                                class="text-sm font-bold {{ $displayReturnMonthly !== null ? ($displayReturnMonthly >= 0 ? 'text-green-600' : 'text-red-600') : 'text-muted' }}">{{ $displayReturnMonthly !== null ? number_format($displayReturnMonthly, 2, ',', '.') . '%' : '—' }}</span>
                        </div>
                        <div class="px-6 py-3.5 flex items-start gap-4"><span
                                class="text-xs font-semibold text-muted w-36 shrink-0">Return YTD</span><span
                                class="text-sm font-bold {{ $displayReturnYtd !== null ? ($displayReturnYtd >= 0 ? 'text-green-600' : 'text-red-600') : 'text-muted' }}">{{ $displayReturnYtd !== null ? number_format($displayReturnYtd, 2, ',', '.') . '%' : '—' }}</span>
                        </div>
                        <div class="px-6 py-3.5 flex items-start gap-4"><span
                                class="text-xs font-semibold text-muted w-36 shrink-0">Return Tahunan</span><span
                                class="text-sm font-bold {{ $displayReturnYearly !== null ? ($displayReturnYearly >= 0 ? 'text-green-600' : 'text-red-600') : 'text-muted' }}">{{ $displayReturnYearly !== null ? number_format($displayReturnYearly, 2, ',', '.') . '%' : '—' }}</span>
                        </div>
                        ...
                        <div class="px-6 py-3.5 flex items-start gap-4"><span
                                class="text-xs font-semibold text-muted w-36 shrink-0">Unit Penyertaan</span><span
                                class="text-sm font-bold text-primary">{{ $unitPenyertaan !== null ? number_format($unitPenyertaan, 0, ',', '.') : '—' }}</span>
                        </div>
                        <div class="px-6 py-3.5 flex items-start gap-4"><span
                                class="text-xs font-semibold text-muted w-36 shrink-0">Maks. Unit Penyertaan</span><span
                                class="text-sm font-bold text-primary">{{ $maxUnit ? number_format($maxUnit, 0, ',', '.') : '—' }}</span>
            ...
            @if ($committees->isNotEmpty() || $investmentManagers->isNotEmpty() || $mi)
                <div class="bg-white rounded-2xl border border-line shadow-sm overflow-hidden mt-6">
                                ...
                                <tbody class="divide-y divide-line">
                                    @foreach ($committees as $mt)
                                        <tr class="hover:bg-[#f8fafc]">
                                            <td class="px-4 py-3 text-xs"><button type="button"
                                                    @click="openPerson({{ Js::from($mt->name) }})"
                                                    class="text-accent hover:underline text-left font-semibold">{{ $mt->name }}</button>
                                            </td>
                                            <td class="px-4 py-3 text-xs text-muted">{{ $mt->position ?? '—' }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                            ...
                            <table class="w-full text-sm">
                                ...
                                <tbody class="divide-y divide-line">
                                    @foreach ($icLines as $line)
                                        ...
                                        <tr class="hover:bg-[#f8fafc]">
                                            <td class="px-4 py-3 text-xs"><button type="button"
                                                    @click="openPerson({{ Js::from($name) }})"
                                                    class="text-accent hover:underline text-left font-semibold">{{ $name }}</button>
                                            </td>
                                            <td class="px-4 py-3 text-xs text-muted">{{ $pos ?: '—' }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                    ...
                    @elseif($mi && $mi->investment_management_team)
                        ...
                        <div class="px-6 py-3 border-b border-line bg-[#f8fafc]">
                            <h3 class="font-semibold text-primary text-xs">Tim Pengelola Investasi (dari Manajer Investasi)
                            </h3>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="w-full text-sm">
                    ...
                    <div
                        ...
                        <div class="flex items-center gap-2">
                            <button @click="toggleLock('ringkasan')" :disabled="toggling"
                                class="px-3 py-1.5 rounded-lg text-xs font-semibold transition flex items-center gap-1.5"
                                :class="isLocked('ringkasan') ? 'bg-amber-400/30 text-amber-100 hover:bg-amber-400/40' :
                                    'bg-white/20 text-white hover:bg-white/30'">
                                <template x-if="isLocked('ringkasan')"><svg class="w-3.5 h-3.5" fill="none"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                                    </svg></template>
                        ...
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="bg-[#f8fafc] text-left text-muted text-xs uppercase tracking-wide">
                                    ...
                                    <th class="px-4 py-3 font-semibold text-right">Unit Penyertaan</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-line">
                                @forelse($navHistory as $nh)
                                    <tr class="hover:bg-[#f8fafc] transition-colors">
                                        ...
                                        <td class="px-4 py-3 text-xs text-right font-semibold text-primary tabular-nums">
                                            {{ number_format($nh->nab_per_unit, 4, ',', '.') }}</td>
                                        <td class="px-4 py-3 text-xs text-right tabular-nums">
                                            {{ $nh->aum ? 'Rp' . number_format($nh->aum, 0, ',', '.') : '—' }}</td>
                                        <td class="px-4 py-3 text-xs text-right tabular-nums">
                                            {{ $nh->unit_participation ? number_format($nh->unit_participation, 0, ',', '.') : '—' }}
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-6 py-12 text-center text-muted">
                                            <p class="font-medium">Belum ada data historis</p>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
                ...
                <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        const formatRupiah = value => {
                            ...
                            if (Math.abs(n) >= 1_000_000_000_000) return 'Rp ' + (n / 1_000_000_000_000).toLocaleString(
                                'id-ID', {
                                    maximumFractionDigits: 2
                                }) + ' T';
                            if (Math.abs(n) >= 1_000_000_000) return 'Rp ' + (n / 1_000_000_000).toLocaleString('id-ID', {
                                maximumFractionDigits: 2
                            }) + ' M';
                        ...
                        const formatUnit = value => {
                            const n = Number(value || 0);
                            if (Math.abs(n) >= 1_000_000_000) return (n / 1_000_000_000).toLocaleString('id-ID', {
                                maximumFractionDigits: 2
                            }) + ' Miliar';
                            if (Math.abs(n) >= 1_000_000) return (n / 1_000_000).toLocaleString('id-ID', {
                                maximumFractionDigits: 2
                            }) + ' Juta';
                        ...
                        if (document.getElementById('chartAum')) {
                            new ApexCharts(document.getElementById('chartAum'), baseOptions(chartData.aum.series, formatRupiah,
                                'aum-bulanan-reksa-dana')).render();
                        }
                        if (document.getElementById('chartUp')) {
                            new ApexCharts(document.getElementById('chartUp'), baseOptions(chartData.up.series, formatUnit,
                                'total-up-bulanan-reksa-dana')).render();
                        }
                        if (document.getElementById('chartNav')) {
                            new ApexCharts(document.getElementById('chartNav'), baseOptions(chartData.nav.series, formatNumber,
                                'nab-up-harian-reksa-dana')).render();
                        }
                    });
                </script>
            @endif
                    ...
                    <div
                        ...
                        <div class="flex items-center gap-2">
                            <button @click="toggleLock('risiko')" :disabled="toggling"
                                class="px-3 py-1.5 rounded-lg text-xs font-semibold transition flex items-center gap-1.5"
                                :class="isLocked('risiko') ? 'bg-amber-400/30 text-amber-100 hover:bg-amber-400/40' :
                                    'bg-white/20 text-white hover:bg-white/30'">
                                <template x-if="isLocked('risiko')"><svg class="w-3.5 h-3.5" fill="none"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                                    </svg></template>
                        ...
                        <div class="divide-y divide-line">
                            @if ($fund->risk_category)
                                <div class="px-6 py-3.5 flex items-start gap-4">
                                    ...
                                    @endphp
                                    <span
                                        class="text-sm px-2 py-0.5 rounded-full text-xs font-semibold {{ $fund->risk_category == 'Rendah' ? 'bg-green-100 text-green-700' : ($fund->risk_category == 'Sedang' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700') }}">{{ $riskLabel }}</span>
                                </div>
                            @endif
                            @if ($fund->conservative_category)
                                <div class="px-6 py-3.5 flex items-start gap-4">
                                    <span class="text-xs font-semibold text-muted w-40 shrink-0">Kategori
                                        Konservatif</span>
                    ...
                    <div class="bg-white rounded-2xl border border-line shadow-sm overflow-hidden">
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-sm">
                                ...
                                <tbody class="divide-y divide-line">
                                    @if (
                                        $riskMetrics['sharpe_ratio_1y'] !== null ||
                                            $riskMetrics['sharpe_ratio_3y'] !== null ||
                                            $riskMetrics['sharpe_ratio_5y'] !== null)
                                        <tr class="hover:bg-[#f8fafc]">
                                            <td class="px-4 py-3 text-xs font-semibold">Sharpe Ratio</td>
                                            <td class="px-4 py-3 text-xs text-right tabular-nums">
                                                {{ $riskMetrics['sharpe_ratio_1y'] !== null ? number_format($riskMetrics['sharpe_ratio_1y'], 4, ',', '.') : '—' }}
                                            </td>
                                            <td class="px-4 py-3 text-xs text-right tabular-nums">
                                                {{ $riskMetrics['sharpe_ratio_3y'] !== null ? number_format($riskMetrics['sharpe_ratio_3y'], 4, ',', '.') : '—' }}
                                            </td>
                                            <td class="px-4 py-3 text-xs text-right tabular-nums">
                                                {{ $riskMetrics['sharpe_ratio_5y'] !== null ? number_format($riskMetrics['sharpe_ratio_5y'], 4, ',', '.') : '—' }}
                                            </td>
                                        </tr>
                                    @endif
                                    @if ($riskMetrics['stdev_1y'] !== null || $riskMetrics['stdev_3y'] !== null || $riskMetrics['stdev_5y'] !== null)
                                        <tr class="hover:bg-[#f8fafc]">
                                            ...
                                            <td class="px-4 py-3 text-xs text-right tabular-nums">
                                                {{ $riskMetrics['stdev_1y'] !== null ? number_format($riskMetrics['stdev_1y'] * 100, 2, ',', '.') . '%' : '—' }}
                                            </td>
                                            <td class="px-4 py-3 text-xs text-right tabular-nums">
                                                {{ $riskMetrics['stdev_3y'] !== null ? number_format($riskMetrics['stdev_3y'] * 100, 2, ',', '.') . '%' : '—' }}
                                            </td>
                                            <td class="px-4 py-3 text-xs text-right tabular-nums">
                                                {{ $riskMetrics['stdev_5y'] !== null ? number_format($riskMetrics['stdev_5y'] * 100, 2, ',', '.') . '%' : '—' }}
                                            </td>
                                        </tr>
                                    @endif
                                    @if ($riskMetrics['beta_1y'] !== null || $riskMetrics['beta_3y'] !== null || $riskMetrics['beta_5y'] !== null)
                                        <tr class="hover:bg-[#f8fafc]">
                                            ...
                                            <td class="px-4 py-3 text-xs text-right tabular-nums">
                                                {{ $riskMetrics['beta_1y'] !== null ? number_format($riskMetrics['beta_1y'], 4, ',', '.') : '—' }}
                                            </td>
                                            <td class="px-4 py-3 text-xs text-right tabular-nums">
                                                {{ $riskMetrics['beta_3y'] !== null ? number_format($riskMetrics['beta_3y'], 4, ',', '.') : '—' }}
                                            </td>
                                            <td class="px-4 py-3 text-xs text-right tabular-nums">
                                                {{ $riskMetrics['beta_5y'] !== null ? number_format($riskMetrics['beta_5y'], 4, ',', '.') : '—' }}
                                            </td>
                                        </tr>
                                    @endif
                                    @if (
                                        $riskMetrics['max_drawdown_1y'] !== null ||
                                            $riskMetrics['max_drawdown_3y'] !== null ||
                                            $riskMetrics['max_drawdown_5y'] !== null)
                                        <tr class="hover:bg-[#f8fafc]">
                                            <td class="px-4 py-3 text-xs font-semibold">Max Drawdown</td>
                                            <td class="px-4 py-3 text-xs text-right tabular-nums">
                                                {{ $riskMetrics['max_drawdown_1y'] !== null ? number_format($riskMetrics['max_drawdown_1y'] * 100, 2, ',', '.') . '%' : '—' }}
                                            </td>
                                            <td class="px-4 py-3 text-xs text-right tabular-nums">
                                                {{ $riskMetrics['max_drawdown_3y'] !== null ? number_format($riskMetrics['max_drawdown_3y'] * 100, 2, ',', '.') . '%' : '—' }}
                                            </td>
                                            <td class="px-4 py-3 text-xs text-right tabular-nums">
                                                {{ $riskMetrics['max_drawdown_5y'] !== null ? number_format($riskMetrics['max_drawdown_5y'] * 100, 2, ',', '.') . '%' : '—' }}
                                            </td>
                                        </tr>
                                    @endif
                                </tbody>
            ...
            <div class="bg-white rounded-2xl border border-line shadow-sm overflow-hidden">
                <div
                    ...
                    <div class="flex items-center gap-2">
                        <button @click="toggleLock('biaya')" :disabled="toggling"
                            class="px-3 py-1.5 rounded-lg text-xs font-semibold transition flex items-center gap-1.5"
                            :class="isLocked('biaya') ? 'bg-amber-400/30 text-amber-100 hover:bg-amber-400/40' :
                                'bg-white/20 text-white hover:bg-white/30'">
                            <template x-if="isLocked('biaya')"><svg class="w-3.5 h-3.5" fill="none"
                                    stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                                </svg></template>
                ...
                @php
                    ...
                    $hasFeeData =
                        ...
                        $_im ||
                        (!$periodActive &&
                            ($fund->minimum_subscription || $fund->minimum_topup || $fund->minimum_redemption));
                @endphp
                @if ($hasFeeData)
                    <div class="divide-y divide-line">
                        @if ($_sub)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-40 shrink-0">Subscription Fee</span><span
                                    class="text-sm">{{ number_format($_sub, 2, ',', '.') }}%</span></div>
                        @endif
                        @if ($_red)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-40 shrink-0">Redemption Fee</span><span
                                    class="text-sm">{{ number_format($_red, 2, ',', '.') }}%</span></div>
                        @endif
                        @if ($_swi)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-40 shrink-0">Switching Fee</span><span
                                    class="text-sm">{{ number_format($_swi, 2, ',', '.') }}%</span></div>
                        @endif
                        @if ($_mgmt)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-40 shrink-0">Management Fee</span><span
                                    class="text-sm">{{ number_format($_mgmt, 2, ',', '.') }}%</span></div>
                        @endif
                        @if ($_cust)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-40 shrink-0">Custodian Fee</span><span
                                    class="text-sm">{{ number_format($_cust, 2, ',', '.') }}%</span></div>
                        @endif
                        @if ($_exp)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-40 shrink-0">Expense Ratio</span><span
                                    class="text-sm">{{ number_format($_exp, 4, ',', '.') }}%</span></div>
                        @endif
                        @if ($_im)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-40 shrink-0">IM Fee</span><span
                                    class="text-sm">{{ $_im }}</span></div>
                        @endif
                        @if (!$periodActive && $fund->minimum_subscription)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-40 shrink-0">Minimum Pembelian</span><span
                                    class="text-sm">Rp{{ number_format($fund->minimum_subscription, 0, ',', '.') }}</span>
                            </div>
                        @endif
                        @if (!$periodActive && $fund->minimum_topup)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-40 shrink-0">Minimum Top Up</span><span
                                    class="text-sm">Rp{{ number_format($fund->minimum_topup, 0, ',', '.') }}</span></div>
                        @endif
                        @if (!$periodActive && $fund->minimum_redemption)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-40 shrink-0">Minimum Redemption</span><span
                                    class="text-sm">Rp{{ number_format($fund->minimum_redemption, 0, ',', '.') }}</span>
                            </div>
                        @endif
                    </div>
                @else
                    <div class="py-12 text-center text-muted text-sm">
                        ...
                        @if ($periodActive)
                            @php $monthNames = ['Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember']; @endphp
                            Data biaya belum tersedia untuk periode {{ $monthNames[$periodMonth - 1] ?? '-' }}
                            {{ $periodYear }}.
                        @else
        ...
        <div x-show="tab === 'portofolio'" x-cloak>
            ...
            @if (!$hasAnyData)
                <div class="py-12 text-center text-muted bg-white rounded-2xl border border-line">
                    ...
                    @if ($periodActive)
                        @php $monthNames = ['Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember']; @endphp
                        <p class="font-medium">Data portofolio belum tersedia untuk periode
                            {{ $monthNames[$periodMonth - 1] ?? '-' }} {{ $periodYear }}.</p>
                    @else
            ...
            @else
                <div class="space-y-6">
                        ...
                        @if ($latestAa)
                            <div class="divide-y divide-line">
                                <div class="px-6 py-3.5 flex items-start gap-4">
                                    ...
                                    <span
                                        class="text-sm font-bold text-primary">{{ number_format($latestAa->equity_percent ?? 0, 2, ',', '.') }}%</span>
                                </div>
                                <div class="px-6 py-3.5 flex items-start gap-4">
                                    <span class="text-xs font-semibold text-muted w-36 shrink-0">Obligasi</span>
                                    <span
                                        class="text-sm font-bold text-primary">{{ number_format($latestAa->bond_percent ?? 0, 2, ',', '.') }}%</span>
                                </div>
                                <div class="px-6 py-3.5 flex items-start gap-4">
                                    <span class="text-xs font-semibold text-muted w-36 shrink-0">Pasar Uang</span>
                                    <span
                                        class="text-sm font-bold text-primary">{{ number_format($latestAa->money_market_percent ?? 0, 2, ',', '.') }}%</span>
                                </div>
                                <div class="px-6 py-3.5 flex items-start gap-4">
                                    <span class="text-xs font-semibold text-muted w-36 shrink-0">Kas</span>
                                    <span
                                        class="text-sm font-bold text-primary">{{ number_format($latestAa->cash_percent ?? 0, 2, ',', '.') }}%</span>
                                </div>
                            </div>
                        @else
                    ...
                    @if ($showAa)
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <div class="bg-white rounded-2xl border border-line shadow-sm p-5">
                                <h3 class="font-bold text-primary text-sm mb-4">Asset Allocation
                                    ({{ $showAa->period_date->format('d M Y') }})</h3>
                                ...
                                @if ($showTopHoldings->isNotEmpty())
                                    <div class="overflow-x-auto">
                                        <table class="w-full text-sm">
                                            <thead>
                                                <tr
                                                    class="bg-[#f8fafc] text-left text-muted text-xs uppercase tracking-wide">
                                                    <th class="px-3 py-2 font-semibold">Efek</th>
                                                    <th class="px-3 py-2 font-semibold">Jenis</th>
                                                    <th class="px-3 py-2 font-semibold text-right">Bobot</th>
                                                </tr>
                                            </thead>
                                            <tbody class="divide-y divide-line">
                                                @foreach ($showTopHoldings as $th)
                                                    <tr class="hover:bg-[#f8fafc]">
                                                        ...
                                                        <td class="px-3 py-2 text-xs text-right font-semibold">
                                                            {{ number_format($th->weight_percent, 2, ',', '.') }}%</td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
                                        </table>
                    ...
                    @if ($portfolioTimeline->isNotEmpty())
                        @php
                            $ptLabels = $portfolioTimeline
                                ->keys()
                                ->map(fn($d) => \Carbon\Carbon::parse($d)->format('d M Y'));
                            $allSecurities = $portfolioTimeline
                                ->flatMap(fn($items) => $items->pluck('security_name'))
                                ->unique()
                                ->values();
                        ...
                        <div class="bg-white rounded-2xl border border-line shadow-sm p-5">
                            <div class="flex items-center justify-between mb-4">
                                ...
                                <button @click="openPortfolioLegendModal()"
                                    class="px-3 py-1.5 bg-accent/10 text-accent rounded-lg text-xs font-semibold hover:bg-accent/20 transition">Detail</button>
                            </div>
                ...
                <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        @if ($showAa)
                            new Chart(document.getElementById('chartAaPie'), {
                                ...
                                data: {
                                    ...
                                    datasets: [{
                                        data: [{{ $showAa->equity_percent ?? 0 }},
                                            {{ $showAa->bond_percent ?? 0 }},
                                            {{ $showAa->money_market_percent ?? 0 }},
                                            {{ $showAa->cash_percent ?? 0 }}
                                        ],
                        ...
                        @if ($aaTimeline->isNotEmpty())
                            new Chart(document.getElementById('chartAaTimeline'), {
                                ...
                                data: {
                                    ...
                                    datasets: [{
                                            label: 'Saham',
                                            data: @json($aaTimeline->pluck('equity_percent')),
                                            backgroundColor: '#2563eb'
                                        },
                                        {
                                            label: 'Obligasi',
                                            data: @json($aaTimeline->pluck('bond_percent')),
                                            backgroundColor: '#059669'
                                        },
                                        {
                                            label: 'Pasar Uang',
                                            data: @json($aaTimeline->pluck('money_market_percent')),
                                            backgroundColor: '#d97706'
                                        },
                                        {
                                            label: 'Kas',
                                            data: @json($aaTimeline->pluck('cash_percent')),
                        ...
                        @if ($portfolioTimeline->isNotEmpty())
                            ...
                            const ptRaw = {!! json_encode(
                                $portfolioTimeline->map(fn($items) => $items->keyBy('security_name')->map(fn($i) => $i->weight_percent)),
                            ) !!};
                            ...
                            securities.forEach((sec, i) => {
                                datasets.push({
                                    ...
                                    data: ptLabels.map((_, idx) => {
                                        ...
                                        return ptRaw[periodKey] && ptRaw[periodKey][sec] ? ptRaw[
                                            periodKey][sec] : 0;
                                    }),
        ...
        {{-- FFS Preview Modal --}}
        <div id="modal-ffs-preview" class="fixed inset-0 z-50 hidden bg-black/40 flex items-center justify-center p-4"
            @click.self="closeFfsPreviewModal()">
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
                <div
                    class="sticky top-0 bg-white border-b border-line p-4 flex items-center justify-between rounded-t-2xl">
                    <h3 class="font-bold text-primary">Preview Data FFS</h3>
                    <button type="button" @click="closeFfsPreviewModal()"
                        class="p-1 hover:bg-[#f1f5f9] rounded-lg transition text-muted">
        ...
        <div x-show="tab === 'pdf-prospektus'" x-cloak x-data="documentTabData('prospektus')">
            @include('admin.daftar-reksa-dana.partials.tab-pdf-document', [
                'fund' => $fund,
                'docType' => 'prospektus',
            ])
        </div>
                ...
                @if ($extractionResults->isNotEmpty())
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="bg-[#f8fafc] text-left text-muted text-xs uppercase tracking-wide">
                                    <th class="px-4 py-3 font-semibold">Tanggal Ekstrak</th>
                                ...
                                @foreach ($extractionResults as $er)
                                    <tr class="hover:bg-[#f8fafc]">
                                        <td class="px-4 py-3 text-xs text-muted">
                                            {{ $er->created_at?->format('d M Y H:i') ?: '—' }}</td>
                                        <td class="px-4 py-3 text-xs font-semibold">
                                            {{ $er->document?->original_name ?: '—' }}</td>
                                        <td class="px-4 py-3 text-xs text-muted">
                                            {{ $er->ffs_month ? str_pad($er->ffs_month, 2, '0', STR_PAD_LEFT) . '/' . $er->ffs_year : '—' }}
                                        </td>
                                        <td class="px-4 py-3 text-xs text-center">
                                            <div class="flex items-center justify-center gap-2">
                                                <button @click="openExtractionDetail({{ $er->id }})"
                                                    class="px-3 py-1.5 bg-accent/10 text-accent rounded-lg text-xs font-semibold hover:bg-accent/20 transition">Lihat
                                                    Data Ekstrak</button>
        ...
        <div x-show="extractionModal.open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4 py-6">
            ...
            <div
                class="relative bg-white rounded-2xl shadow-xl border border-line w-full max-w-2xl max-h-[85vh] overflow-y-auto">
                <div class="px-6 py-4 border-b border-line flex items-center justify-between">
                    ...
                    <button type="button" @click="extractionModal.open = false"
                        class="text-muted hover:text-primary text-xl leading-none">&times;</button>
                </div>
                <div class="p-6">
                    ...
                    <template x-if="extractionModal.error">
                        <div class="px-4 py-3 rounded-xl text-sm bg-red-50 border border-red-200 text-red-700"
                            x-text="extractionModal.error"></div>
                    </template>
                    <template x-if="!extractionModal.loading && !extractionModal.error">
                        <div>
                                ...
                                <template x-for="(item, idx) in extractionModal.items" :key="idx">
                                    <div>
                                        <template x-if="item.type === 'simple'">
                                            <div
                                                class="flex items-start gap-3 px-4 py-2.5 bg-[#f8fafc] rounded-lg border border-line">
                                                <span class="text-xs font-semibold text-muted w-36 shrink-0"
                                        ...
                                        <template x-if="item.type === 'table'">
                                            <div class="bg-[#f8fafc] rounded-lg border border-line p-3 overflow-x-auto">
                                                ...
                                                <table class="w-full text-xs">
                                                    <thead>
                                                        <tr class="bg-white">
                                                            <template x-for="(h, hi) in item.headers"
                                                                :key="hi">
                                                                <th class="px-2 py-1.5 text-left font-semibold text-muted"
                                                                    x-text="h"></th>
                                                            </template>
                                                        </tr>
                                                    </thead>
                            ...
                            <div x-show="extractionModal.items.length === 0" class="py-8 text-center text-muted text-sm">
                                Tidak ada data ekstrak.</div>
                        </div>
                    </template>
                </div>
            </div>
        ...
        <div x-show="portfolioLegendModal.open" x-cloak
            class="fixed inset-0 z-50 flex items-center justify-center px-4 py-6">
            <div class="absolute inset-0 bg-black/40" @click="portfolioLegendModal.open = false"></div>
            <div
                class="relative bg-white rounded-2xl shadow-xl border border-line w-full max-w-lg max-h-[85vh] overflow-y-auto">
                <div class="px-6 py-4 border-b border-line flex items-center justify-between">
                    <h2 class="font-bold text-primary">Detail Portfolio Composition</h2>
                    <button type="button" @click="portfolioLegendModal.open = false"
                        class="text-muted hover:text-primary text-xl leading-none">&times;</button>
                </div>
                <div class="p-6">
                    @if ($showTopHoldings->isNotEmpty())
                                    ...
                                    <tr class="hover:bg-[#f8fafc]">
                                        <td class="px-3 py-2 text-xs font-semibold flex items-center gap-2">
                                            <span class="w-3 h-3 rounded-full inline-block shrink-0"
                                                style="background-color: {{ $ptColors[$loop->index % count($ptColors)] }}"></span>
                                            {{ $th->security_name }}
                                        </td>
        ...
        <div x-show="personModal.open" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4 py-6">
            ...
            <div
                class="relative bg-white rounded-2xl shadow-xl border border-line w-full max-w-3xl max-h-[85vh] overflow-y-auto">
                <div class="px-6 py-4 border-b border-line flex items-center justify-between">
                    <div>
                        <p class="text-xs text-muted">Detail keterkaitan</p>
                        <h2 class="font-bold text-primary" x-text="personModal.data?.name || 'Memuat...'"></h2>
                    </div>
                    <button type="button" @click="personModal.open = false"
                        class="text-muted hover:text-primary text-xl leading-none">&times;</button>
                </div>
                <div class="p-6 space-y-6">
                    <div x-show="personModal.loading" class="text-sm text-muted">Memuat data...</div>
                    <div x-show="personModal.error" x-text="personModal.error"
                            ...
                            <div>
                                ...
                                <div class="overflow-x-auto" x-show="personModal.data.funds.length > 0">
                                    <table class="w-full text-sm">
                                        <thead>
                                            <tr class="bg-[#f8fafc] text-left text-muted text-xs uppercase tracking-wide">
                                                <th class="px-3 py-2">Reksa Dana</th>
                                        ...
                                        <tbody class="divide-y divide-line"><template
                                                x-for="row in personModal.data.funds"
                                                :key="row.name + row.role + row.position">
                                                <tr>
                                                    <td class="px-3 py-2 font-semibold" x-text="row.name"></td>
                                                    <td class="px-3 py-2 font-mono text-xs" x-text="row.code || '-'"></td>
                                                    <td class="px-3 py-2" x-text="row.role || '-'"></td>
                                                    <td class="px-3 py-2 text-muted" x-text="row.position || '-'"></td>
                                                    <td class="px-3 py-2 text-xs text-muted" x-text="row.source || '-'">
                                                    </td>
                                                </tr>
                                            </template></tbody>
                                    </table>
                            ...
                            <div>
                                ...
                                <div class="overflow-x-auto" x-show="personModal.data.managers.length > 0">
                                    <table class="w-full text-sm">
                                        <thead>
                                            <tr class="bg-[#f8fafc] text-left text-muted text-xs uppercase tracking-wide">
                                                <th class="px-3 py-2">Manajer Investasi</th>
                                                <th class="px-3 py-2">Peran</th>
                                                <th class="px-3 py-2">Jabatan</th>
                                                <th class="px-3 py-2">Sumber</th>
                                            </tr>
                                        ...
                                        <tbody class="divide-y divide-line"><template
                                                x-for="row in personModal.data.managers"
                                                :key="row.name + row.role + row.position">
                                                <tr>
                                                    <td class="px-3 py-2 font-semibold" x-text="row.name"></td>
                                                    <td class="px-3 py-2" x-text="row.role || '-'"></td>
                                ...
                                <div class="space-y-2" x-show="personModal.data.news.length > 0">
                                    <template x-for="item in personModal.data.news" :key="item.url || item.title">
                                        <a :href="item.url" target="_blank"
                                            class="block border border-line rounded-xl px-4 py-3 hover:border-accent transition">
                                            <p class="text-sm font-semibold text-primary" x-text="item.title"></p>
                                            <p class="text-xs text-muted mt-1"><span x-text="item.source || '-'"></span>
                                                <span x-show="item.published_at">-</span> <span
                                                    x-text="item.published_at || ''"></span></p>
                                        </a>
                                    </template>
                                </div>
                            </div>
        ...
        <div x-show="editModal === 'ringkasan'" x-cloak
            class="fixed inset-0 z-50 flex items-center justify-center px-4 py-6" @click.self="editModal = null">
            ...
            <div
                ...
                <form @submit.prevent="submitEdit('ringkasan')" class="p-6 space-y-4">
                    <div>
                        ...
                        <input type="number" step="0.0001" x-model="editData.nab_per_unit"
                    ...
                    <div>
                        ...
                        <input type="date" x-model="editData.tanggal_nab"
                            class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-primary mb-1">AUM</label>
                        <input type="number" step="0.01" x-model="editData.aum"
                            class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-primary mb-1">Total Unit Penyertaan</label>
                        <input type="number" step="0.01" x-model="editData.total_unit"
                            class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                    </div>
                    <div class="flex justify-end gap-2 pt-2">
                        <button type="button" @click="editModal = null"
                            class="px-4 py-2 text-sm text-muted border border-line rounded-lg hover:bg-gray-50">Batal</button>
                        <button type="submit"
                            class="px-4 py-2 text-sm text-white bg-emerald-700 rounded-lg hover:bg-emerald-800">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
            ...
            <div
                ...
                <form @submit.prevent="submitEdit('risiko')" class="p-6 space-y-4">
                    <div>
                        ...
                        <select x-model="editData.risk_category"
                            class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                            <option value="">—</option>
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        <div>
                            ...
                            <input type="number" step="0.0001" x-model="editData.sharpe_ratio_1y"
                                class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-primary mb-1">Sharpe 3Th</label>
                            <input type="number" step="0.0001" x-model="editData.sharpe_ratio_3y"
                                class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-primary mb-1">Sharpe 5Th</label>
                            <input type="number" step="0.0001" x-model="editData.sharpe_ratio_5y"
                                class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                        </div>
                    </div>
                    <div class="grid grid-cols-3 gap-3">
                        <div>
                            ...
                            <input type="number" step="0.0001" x-model="editData.stdev_1y"
                                class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-primary mb-1">Std Dev 3Th</label>
                            <input type="number" step="0.0001" x-model="editData.stdev_3y"
                                class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-primary mb-1">Std Dev 5Th</label>
                            <input type="number" step="0.0001" x-model="editData.stdev_5y"
                                class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                        </div>
                    </div>
                    <div class="grid grid-cols-3 gap-3">
                        <div>
                            ...
                            <input type="number" step="0.0001" x-model="editData.beta_1y"
                                class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-primary mb-1">Beta 3Th</label>
                            <input type="number" step="0.0001" x-model="editData.beta_3y"
                                class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-primary mb-1">Beta 5Th</label>
                            <input type="number" step="0.0001" x-model="editData.beta_5y"
                                class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                        </div>
                    </div>
                    <div class="grid grid-cols-3 gap-3">
                        <div>
                            ...
                            <input type="number" step="0.0001" x-model="editData.max_drawdown_1y"
                                class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-primary mb-1">Max DD 3Th</label>
                            <input type="number" step="0.0001" x-model="editData.max_drawdown_3y"
                                class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-primary mb-1">Max DD 5Th</label>
                            <input type="number" step="0.0001" x-model="editData.max_drawdown_5y"
                                class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                        </div>
                    </div>
                    <div class="flex justify-end gap-2 pt-2">
        ...
        {{-- Modal Edit Biaya --}}
        <div x-show="editModal === 'biaya'" x-cloak
            class="fixed inset-0 z-50 flex items-center justify-center px-4 py-6" @click.self="editModal = null">
            <div class="absolute inset-0 bg-black/40" @click="editModal = null"></div>
            <div
                class="relative bg-white rounded-2xl shadow-xl border border-line w-full max-w-lg max-h-[85vh] overflow-y-auto">
                ...
                <form @submit.prevent="submitEdit('biaya')" class="p-6 space-y-4">
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="number" step="0.01" x-model="editData.subscription_fee"
                                class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-primary mb-1">Redemption Fee (%)</label>
                            <input type="number" step="0.01" x-model="editData.redemption_fee"
                                class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="number" step="0.01" x-model="editData.switching_fee"
                                class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-primary mb-1">Management Fee (%)</label>
                            <input type="number" step="0.01" x-model="editData.management_fee"
                                class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-3">
        ...
        <div x-show="editModal === 'portofolio'" x-cloak
            class="fixed inset-0 z-50 flex items-center justify-center px-4 py-6" @click.self="editModal = null">
            <div class="absolute inset-0 bg-black/40" @click="editModal = null"></div>
            <div
                class="relative bg-white rounded-2xl shadow-xl border border-line w-full max-w-2xl max-h-[85vh] overflow-y-auto">
                ...
                <form @submit.prevent="savePortfolio()" class="p-6 space-y-4">
                    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                        <div>
                            ...
                            <input type="number" step="0.01" x-model="portfolioSaham"
                                class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-primary mb-1">Obligasi (%)</label>
                            <input type="number" step="0.01" x-model="portfolioObligasi"
                                class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-primary mb-1">Pasar Uang (%)</label>
                            <input type="number" step="0.01" x-model="portfolioPasarUang"
                                class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-primary mb-1">Kas (%)</label>
                            <input type="number" step="0.01" x-model="portfolioKas"
                                class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                        </div>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-primary mb-1">Top Holdings <span
                                class="text-[10px] text-muted/60">(format: NamaEfek:Bobot:Jenis, tiap baris 1
                                efek)</span></label>
                        <textarea x-model="portfolioTopHoldings" rows="5"
                            class="w-full border border-line rounded-lg px-3 py-2 text-sm font-mono"></textarea>
                    </div>
                    <div class="border-t border-line pt-4">
                        ...
                        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                            <div>
                                <label class="block text-xs font-semibold text-primary mb-1">NAB/UP</label>
                                <input type="number" step="0.0001" x-model="portfolioNabPerUnit"
                                    class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-primary mb-1">Tanggal NAB</label>
                                <input type="date" x-model="portfolioTanggalNab"
                                    class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-primary mb-1">AUM</label>
                                <input type="number" step="0.01" x-model="portfolioAum"
                                    class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-primary mb-1">Total Unit</label>
                                <input type="number" step="0.01" x-model="portfolioTotalUnit"
                                    class="w-full border border-line rounded-lg px-3 py-2 text-sm">
                            </div>
                        </div>
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-3 pt-2">
                        ...
                        <div class="flex gap-2">
                            <button type="button"
                                @click="editModal = null; portfolioSuccess = null; portfolioError = null"
                                class="px-4 py-2 text-sm text-muted border border-line rounded-lg hover:bg-gray-50">Batal</button>
                            <button type="submit" :disabled="portfolioSaving"
                                class="px-4 py-2 text-sm text-white bg-emerald-700 rounded-lg hover:bg-emerald-800 disabled:opacity-50">
    ...
    <script>
            ...
            return {
                ...
                openEdit(section) {
                    ...
                    if (section === 'ringkasan') {
                        this.editData = {
                            nab_per_unit: @js($displayNab),
                            tanggal_nab: @js($periodActive && $snapshot?->period_date ? $snapshot->period_date->format('Y-m-d') : ($periodActive && $nav?->tanggal ? $nav->tanggal->format('Y-m-d') : $fund->tanggal_nab?->format('Y-m-d'))),
                            aum: @js($displayAum),
                            total_unit: @js($displayTotalUnit)
                        };
                    } else if (section === 'risiko') {
                        this.editData = {
                            risk_category: @js($fund->risk_category),
                            sharpe_ratio_1y: @js($riskMetrics['sharpe_ratio_1y']),
                            sharpe_ratio_3y: @js($riskMetrics['sharpe_ratio_3y']),
                            sharpe_ratio_5y: @js($riskMetrics['sharpe_ratio_5y']),
                            stdev_1y: @js($riskMetrics['stdev_1y']),
                            stdev_3y: @js($riskMetrics['stdev_3y']),
                            stdev_5y: @js($riskMetrics['stdev_5y']),
                            beta_1y: @js($riskMetrics['beta_1y']),
                            beta_3y: @js($riskMetrics['beta_3y']),
                            beta_5y: @js($riskMetrics['beta_5y']),
                            max_drawdown_1y: @js($riskMetrics['max_drawdown_1y']),
                            max_drawdown_3y: @js($riskMetrics['max_drawdown_3y']),
                            max_drawdown_5y: @js($riskMetrics['max_drawdown_5y']),
                        };
                ...
                isLocked(section) {
                    return this.parserLocks.includes(section)
                },
                ...
                renderFfsPreviewContent() {
                    ...
                    if (this.ffsPreviewData.extracted?.length) {
                        html +=
                            `<div class="bg-white border border-line rounded-lg p-4"><h5 class="font-semibold text-primary mb-2">Data yang Terektrak (${extracted.length} item)</h5><div class="flex flex-wrap gap-1">${extracted.map(e => `<span class="px-2 py-1 bg-blue-50 text-blue-700 text-xs rounded-full">${e}</span>`).join('')}</div></div>`;
                    }
                    ...
                    content.querySelector('[data-ffs-cancel]')?.addEventListener('click', () => this
                .closeFfsPreviewModal());
                ...
                renderPreviewField(label, value, suffix = '') {
                    ...
                    const formatted = typeof value === 'number' ? value.toLocaleString('id-ID', {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2
                    }) : value;
                    return `<div><p class="text-xs text-muted">${label}</p><p class="font-semibold text-primary">${formatted}${suffix}</p></div>`;
                },
                portfolioLegendModal: {
                    open: false
                },
                openPortfolioLegendModal() {
                    this.portfolioLegendModal.open = true;
                },
                extractionModal: {
                    open: false,
                    loading: false,
                ...
                async openExtractionDetail(id) {
                    ...
                    try {
                        ...
                        const isEmpty = v => v === null || v === undefined || v === '' || (Array.isArray(v) && v
                            .length === 0) || (typeof v === 'object' && !Array.isArray(v) && Object.keys(v)
                            .length === 0);
                        this.extractionModal.items = Object.entries(data)
                            ...
                            .map(([key, val]) => {
                                ...
                                if (Array.isArray(val)) {
                                    ...
                                    const values = val.map(v => v === null || v === undefined ? '—' : (typeof v ===
                                        'object' ? JSON.stringify(v) : String(v)));
                                    return {
                                        type: 'list',
                                        key: label,
                                        values
                                ...
                                if (typeof val === 'object' && val !== null) {
                                    ...
                                    const rows = [headers.map(h => {
                                        ...
                                        if (v === null || v === undefined || v === '') return null;
                                        if (typeof v === 'object') return JSON.stringify(v);
                                        return typeof v === 'number' ? Number(v).toLocaleString(
                                            'id-ID') : String(v);
                                    })];
                                    return {
                                        type: 'table',
                                ...
                                const display = typeof val === 'number' ? Number(val).toLocaleString('id-ID') :
                                    String(val);
                                return {
                                    type: 'simple',
                                    key: label,
        ...
        @php
            $partitionsByDoc = $fund->documents->mapWithKeys(fn($d) => [
                $d->id => $d->partitions->map(fn($p) => ['id' => $p->id, 'start_page' => $p->start_page, 'end_page' => $p->end_page])->keyBy('id'),
            ]);
        @endphp
        function documentTabData(defaultDocType) {
            return {
                ...
                ffsError: {},
                pageContentCache: {},
                partitionsByDoc: @json($partitionsByDoc),
                ...
                async showPageContent(docId, pageId) {
                    ...
                    try {
                        ...
                        if (page) {
                            this.selectedPageContent = '<pre class="text-xs whitespace-pre-wrap">' + this.escapeHtml(
                                page.text_content) + '</pre>';
                            this.pageContentCache[cacheKey] = this.selectedPageContent;
                        }
                    } catch (e) {
                        this.selectedPageContent = '<span class="text-red-600">' + this.escapeHtml(e.message) +
                            '</span>';
                    }
                },
                ...
                async uploadFfs() {
                    ...
                    try {
                        ...
                        const json = await res.json();
                        if (!res.ok) throw new Error(json.error || json.errors?.[Object.keys(json.errors || {})[0]]?.[
                            0] || 'Gagal upload.');
