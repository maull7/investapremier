<div x-show="showImportHarga" x-cloak
    ...
    <div class="absolute inset-0 bg-black/40" @click="showImportHarga = false"></div>
    <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6"
        ...
        <h3 class="font-bold text-primary text-base mb-1">Import Obligasi Harga Referensi</h3>
        <p class="text-muted text-sm mb-4">Upload file Excel sesuai format template. Data dengan kode yang sama akan diperbarui.</p>
        <form method="POST" action="{{ route('admin.obligasi.import-harga-referensi') }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="flex items-center justify-between">
                <a href="{{ route('admin.obligasi.template-harga-referensi') }}" class="text-xs text-accent hover:underline flex items-center gap-1">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                    Download template
                </a>
                <div class="flex items-center gap-2">
                    <button type="button" @click="showImportHarga = false" class="px-4 py-2 border border-line text-muted rounded-xl text-sm font-semibold hover:text-primary transition">Batal</button>
