...
<div class="mb-5 flex items-center justify-between gap-3 flex-wrap">
    <form method="GET" action="{{ route('admin.obligasi.index') }}" class="flex items-center gap-3 flex-1">
        ...
        @if(request('search'))<a href="{{ route('admin.obligasi.index', ['tab' => 'harga-referensi']) }}" class="px-4 py-2.5 border border-line text-muted rounded-xl text-sm font-semibold hover:text-primary transition">Reset</a>@endif
    </form>
                ...
                <tr class="hover:bg-[#f8fafc] transition-colors">
                    ...
                    <td class="px-4 py-3 text-right">
                        <div class="flex items-center justify-end gap-1">
                            <a href="{{ route('admin.obligasi.edit-harga-referensi', $o) }}"
                            ...
                            <button type="button"
                                    @click="deleteId = {{ $o->id }}; deleteType = 'harga-referensi'; deleteText = '{{ addslashes($o->kode . ' - ' . Str::limit($o->nama ?: $o->emiten, 60)) }}'"
