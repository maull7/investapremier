@extends('layouts.admin')

@section('title', 'Sekuritas Informasi - InvestaPremier')

@section('content')
    <div x-data="{ showForm: false, editItem: null, deleteId: null, deleteText: '', showImport: false, importType: 'government' }">

        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="page-title">Sekuritas Informasi</h1>
                <p class="page-sub">Kelola data obligasi dan sukuk pemerintah & korporasi</p>
            </div>
            <div class="flex items-center gap-2">
                <a href="{{ route('admin.sekuritas-informasi.template', ['type' => $tab]) }}"
                    class="btn-secondary">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                    </svg>
                    Template Excel
                </a>
                <button @click="showImport = true; importType = '{{ $tab }}'"
                    class="btn-outline">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                    </svg>
                    Import Excel
                </button>
                <button @click="showForm = true; editItem = null"
                    class="btn-primary">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                    </svg>
                    Tambah Data
                </button>
            </div>
        </div>

        @if (session('success'))
            <div class="alert-success">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                {{ session('success') }}
            </div>
        @endif

        <div class="mb-5">
            <div class="flex items-center gap-1 bg-[#f1f5f9] rounded-xl p-1 w-fit">
                <a href="{{ route('admin.sekuritas-informasi.index', ['tab' => 'government']) }}"
                    class="px-4 py-2 rounded-lg text-sm font-semibold transition {{ $tab === 'government' ? 'bg-white text-primary shadow-sm' : 'text-muted hover:text-primary' }}">
                    Government Bond & Sukuk
                </a>
                <a href="{{ route('admin.sekuritas-informasi.index', ['tab' => 'corporate']) }}"
                    class="px-4 py-2 rounded-lg text-sm font-semibold transition {{ $tab === 'corporate' ? 'bg-white text-primary shadow-sm' : 'text-muted hover:text-primary' }}">
                    Corporate Bond & Sukuk
                </a>
            </div>
        </div>

        <div class="mb-5">
            <form method="GET" action="{{ route('admin.sekuritas-informasi.index') }}">
                <input type="hidden" name="tab" value="{{ $tab }}">
                <div class="flex items-center gap-3">
                    <div class="relative flex-1 max-w-md">
                        <svg class="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-muted" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                        </svg>
                        <input type="text" name="search" value="{{ request('search') }}"
                            class="w-full pl-10 pr-4 py-2.5 border border-line rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent"
                            placeholder="Cari kode, nama, atau ISIN...">
                    </div>
                    <button type="submit"
                        class="px-4 py-2.5 bg-primary text-white rounded-xl text-sm font-semibold hover:bg-primary/90 transition">Cari</button>
                    @if (request('search'))
                        <a href="{{ route('admin.sekuritas-informasi.index', ['tab' => $tab]) }}"
                            class="px-4 py-2.5 border border-line text-muted rounded-xl text-sm font-semibold hover:text-primary transition">Reset</a>
                    @endif
                </div>
            </form>
        </div>

        <div class="table-card">
            <div class="table-head">
                <h2 class="th-title">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
                    </svg>
                    {{ $tab === 'government' ? 'Government Bond & Sukuk' : 'Corporate Bond & Sukuk' }}
                </h2>
                <div class="flex items-center gap-2">
                    <span class="th-meta">Tampilkan:</span>
                    <form method="GET" action="{{ route('admin.sekuritas-informasi.index') }}">
                        <input type="hidden" name="tab" value="{{ $tab }}">
                        @if (request('search'))
                            <input type="hidden" name="search" value="{{ request('search') }}">
                        @endif
                        <select name="per_page" onchange="this.form.submit()"
                            class="text-xs bg-white text-slate-500 border border-white/20 rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-secondary/50">
                            @foreach ([10, 25, 50] as $n)
                                <option value="{{ $n }}" {{ $perPage == $n ? 'selected' : '' }}>
                                    {{ $n }}</option>
                            @endforeach
                        </select>
                    </form>
                    <span class="th-meta">{{ $data->total() }} total</span>
                </div>
            </div>

            @if ($data->isEmpty())
                <div class="py-16 text-center text-muted">
                    <svg class="w-12 h-12 mx-auto mb-3 opacity-30" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                            d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
                    </svg>
                    <p class="font-medium">Belum ada data</p>
                    <p class="text-sm mt-1">Klik "Tambah Data" atau import dari Excel untuk mulai mengisi data</p>
                </div>
            @else
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="bg-[#f8fafc] text-left text-muted text-xs uppercase tracking-wide">
                                <th class="px-4 py-3.5 font-semibold">Kode Obligasi</th>
                                <th class="px-4 py-3.5 font-semibold">Nama Obligasi</th>
                                <th class="px-4 py-3.5 font-semibold">ISIN Code</th>
                                <th class="px-4 py-3.5 font-semibold">Currency</th>
                                <th class="px-4 py-3.5 font-semibold text-right">Outstanding Amount</th>
                                <th class="px-4 py-3.5 font-semibold text-right">Coupon (%)</th>
                                <th class="px-4 py-3.5 font-semibold text-right">Maturity Date</th>
                                <th class="px-4 py-3.5 font-semibold text-right">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-line">
                            @foreach ($data as $item)
                                <tr class="hover:bg-[#f8fafc] transition-colors">
                                    <td class="px-4 py-3">
                                        <div
                                            class="w-fit h-7 rounded-lg bg-primary/10 text-primary font-bold text-xs grid place-items-center px-2">
                                            {{ $item->kode_obligasi }}</div>
                                    </td>
                                    <td class="px-4 py-3">
                                        <span class="font-semibold text-primary leading-snug">{{ $item->nama_obligasi }}</span>
                                    </td>
                                    <td class="px-4 py-3">
                                        <span class="text-xs text-muted font-mono">{{ $item->isin_code ?: '-' }}</span>
                                    </td>
                                    <td class="px-4 py-3">
                                        <span class="text-xs text-muted">{{ $item->currency ?: '-' }}</span>
                                    </td>
                                    <td class="px-4 py-3 text-right font-semibold">
                                        {{ $item->outstanding_amount ? 'Rp' . number_format($item->outstanding_amount, 0, ',', '.') : '-' }}
                                    </td>
                                    <td class="px-4 py-3 text-right">
                                        {{ $item->coupon !== null ? number_format($item->coupon, 3, ',', '.') . '%' : '-' }}
                                    </td>
                                    <td class="px-4 py-3 text-right text-xs text-muted">
                                        {{ $item->maturity_date ? $item->maturity_date->format('d/m/Y') : '-' }}
                                    </td>
                                    <td class="px-4 py-3 text-right">
                                        <div class="flex items-center justify-end gap-1">
                                            <button type="button"
                                                @click="showForm = true; editItem = {{ $item->toJson() }}"
                                                class="p-2 rounded-lg text-muted hover:text-primary hover:bg-[#f1f5f9] transition"
                                                title="Edit">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                </svg>
                                            </button>
                                            <button type="button"
                                                @click="deleteId = {{ $item->id }}; deleteText = '{{ addslashes($item->kode_obligasi . ' - ' . Str::limit($item->nama_obligasi, 60)) }}'"
                                                class="p-2 rounded-lg text-muted hover:text-red-500 hover:bg-red-50 transition"
                                                title="Hapus">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                                </svg>
                                            </button>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                </div>

                @if ($data->hasPages())
                    <div class="px-6 py-4 border-t border-line flex items-center justify-between gap-4 text-sm">
                        <p class="text-muted text-xs">
                            Menampilkan {{ $data->firstItem() }}–{{ $data->lastItem() }} dari {{ $data->total() }}
                            data
                        </p>
                        <div class="flex items-center gap-1">
                            @if ($data->onFirstPage())
                                <span class="px-3 py-1.5 rounded-lg text-muted/40 text-xs cursor-not-allowed">← Prev</span>
                            @else
                                <a href="{{ $data->previousPageUrl() }}"
                                    class="px-3 py-1.5 rounded-lg text-muted hover:text-primary hover:bg-[#f1f5f9] transition text-xs">←
                                    Prev</a>
                            @endif

                            @php
                                $current = $data->currentPage();
                                $last = $data->lastPage();
                                $start = max(1, $current - 2);
                                $end = min($last, $current + 2);
                            @endphp
                            @if ($start > 1)
                                <a href="{{ $data->url(1) }}"
                                    class="w-8 h-8 rounded-lg grid place-items-center text-xs font-semibold text-muted hover:text-primary hover:bg-[#f1f5f9] transition">1</a>
                                @if ($start > 2)
                                    <span class="px-1 text-muted text-xs">…</span>
                                @endif
                            @endif
                            @foreach ($data->getUrlRange($start, $end) as $page => $url)
                                <a href="{{ $url }}"
                                    class="w-8 h-8 rounded-lg grid place-items-center text-xs font-semibold transition
                      {{ $page == $current ? 'bg-primary text-white' : 'text-muted hover:text-primary hover:bg-[#f1f5f9]' }}">
                                    {{ $page }}
                                </a>
                            @endforeach
                            @if ($end < $last)
                                @if ($end < $last - 1)
                                    <span class="px-1 text-muted text-xs">…</span>
                                @endif
                                <a href="{{ $data->url($last) }}"
                                    class="w-8 h-8 rounded-lg grid place-items-center text-xs font-semibold text-muted hover:text-primary hover:bg-[#f1f5f9] transition">{{ $last }}</a>
                            @endif

                            @if ($data->hasMorePages())
                                <a href="{{ $data->nextPageUrl() }}"
                                    class="px-3 py-1.5 rounded-lg text-muted hover:text-primary hover:bg-[#f1f5f9] transition text-xs">Next
                                    →</a>
                            @else
                                <span class="px-3 py-1.5 rounded-lg text-muted/40 text-xs cursor-not-allowed">Next →</span>
                            @endif
                        </div>
                    </div>
                @endif
            @endif
        </div>

        {{-- Modal Form Tambah/Edit --}}
        <div x-show="showForm" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4"
            x-transition:enter="transition duration-200" x-transition:enter-start="opacity-0"
            x-transition:enter-end="opacity-100" x-transition:leave="transition duration-150"
            x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
            <div class="absolute inset-0 bg-black/40" @click="showForm = false"></div>
            <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-lg p-6"
                x-transition:enter="transition duration-200" x-transition:enter-start="opacity-0 scale-95"
                x-transition:enter-end="opacity-100 scale-100">

                <template x-if="!editItem">
                    <form method="POST" action="{{ route('admin.sekuritas-informasi.store') }}">
                        <input type="hidden" name="type" value="{{ $tab }}">
                </template>
                <template x-if="editItem">
                    <form method="POST" :action="`/admin/sekuritas-informasi/${editItem.id}`">
                        @method('PUT')
                        <input type="hidden" name="type" x-model="editItem.type">
                </template>

                @csrf

                <h3 class="font-bold text-primary text-base mb-1" x-text="editItem ? 'Edit Data' : 'Tambah Data'"></h3>
                <p class="text-muted text-sm mb-4" x-text="editItem ? 'Ubah data obligasi / sukuk.' : 'Masukkan data obligasi / sukuk baru.'"></p>

                <div class="space-y-3">
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-xs font-semibold text-primary mb-1">Kode Obligasi *</label>
                            <input type="text" name="kode_obligasi" x-model="editItem?.kode_obligasi"
                                class="w-full border border-line rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-accent/30"
                                required>
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-primary mb-1">ISIN Code</label>
                            <input type="text" name="isin_code" x-model="editItem?.isin_code"
                                class="w-full border border-line rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-accent/30">
                        </div>
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-primary mb-1">Nama Obligasi *</label>
                        <input type="text" name="nama_obligasi" x-model="editItem?.nama_obligasi"
                            class="w-full border border-line rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-accent/30"
                            required>
                    </div>

                    <div class="grid grid-cols-3 gap-3">
                        <div>
                            <label class="block text-xs font-semibold text-primary mb-1">Currency</label>
                            <input type="text" name="currency" x-model="editItem?.currency"
                                class="w-full border border-line rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-accent/30"
                                placeholder="IDR">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-primary mb-1">Outstanding Amount</label>
                            <input type="text" name="outstanding_amount" x-model="editItem?.outstanding_amount"
                                class="w-full border border-line rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-accent/30"
                                placeholder="2417000000000">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-primary mb-1">Coupon (%)</label>
                            <input type="number" step="0.0001" name="coupon" x-model="editItem?.coupon"
                                class="w-full border border-line rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-accent/30"
                                placeholder="12.000">
                        </div>
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-primary mb-1">Maturity Date</label>
                        <input type="date" name="maturity_date" x-model="editItem?.maturity_date"
                            class="w-full border border-line rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-accent/30">
                    </div>
                </div>

                <div class="flex items-center justify-end gap-3 mt-6">
                    <button type="button" @click="showForm = false"
                        class="px-4 py-2 border border-line text-muted rounded-xl text-sm font-semibold hover:text-primary transition">
                        Batal
                    </button>
                    <button type="submit"
                        class="px-4 py-2 bg-accent text-white rounded-xl text-sm font-semibold hover:bg-accent/90 transition"
                        x-text="editItem ? 'Simpan Perubahan' : 'Simpan'">
                    </button>
                </div>
                </form>
            </div>
        </div>

        {{-- Modal Import Excel --}}
        <div x-show="showImport" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4"
            x-transition:enter="transition duration-200" x-transition:enter-start="opacity-0"
            x-transition:enter-end="opacity-100" x-transition:leave="transition duration-150"
            x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
            <div class="absolute inset-0 bg-black/40" @click="showImport = false"></div>
            <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6"
                x-transition:enter="transition duration-200" x-transition:enter-start="opacity-0 scale-95"
                x-transition:enter-end="opacity-100 scale-100">
                <h3 class="font-bold text-primary text-base mb-1">Import Excel</h3>
                <p class="text-muted text-sm mb-4">Upload file Excel sesuai format template. Data dengan kode yang sama akan diperbarui.</p>

                <form method="POST" action="{{ route('admin.sekuritas-informasi.import') }}" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="type" x-model="importType">

                    <div class="border-2 border-dashed border-line rounded-xl p-6 text-center mb-4 hover:border-accent/40 transition">
                        <svg class="w-8 h-8 mx-auto text-muted mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                d="M9 13h6m-3-3v6m5 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                        </svg>
                        <label class="cursor-pointer">
                            <span class="text-sm font-semibold text-accent">Pilih file</span>
                            <span class="text-sm text-muted"> atau drag & drop</span>
                            <input type="file" name="file" accept=".xlsx,.xls,.csv" class="hidden" required>
                        </label>
                        <p class="text-xs text-muted mt-1">Format: .xlsx, .xls, .csv</p>
                    </div>
                    @error('file')
                        <p class="text-red-500 text-xs mb-3">{{ $message }}</p>
                    @enderror

                    <div class="flex items-center justify-between">
                        <a :href="`/admin/sekuritas-informasi/template?type=${importType}`"
                            class="text-xs text-accent hover:underline flex items-center gap-1">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                            </svg>
                            Download template
                        </a>
                        <div class="flex items-center gap-2">
                            <button type="button" @click="showImport = false"
                                class="px-4 py-2 border border-line text-muted rounded-xl text-sm font-semibold hover:text-primary transition">
                                Batal
                            </button>
                            <button type="submit"
                                class="px-4 py-2 bg-accent text-white rounded-xl text-sm font-semibold hover:bg-accent/90 transition">
                                Upload & Import
                            </button>
                        </div>
                    </div>
                </form>
            </div>
        </div>

        {{-- Modal Konfirmasi Hapus --}}
        <div x-show="deleteId !== null" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4"
            x-transition:enter="transition duration-200" x-transition:enter-start="opacity-0"
            x-transition:enter-end="opacity-100" x-transition:leave="transition duration-150"
            x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
            <div class="absolute inset-0 bg-black/40" @click="deleteId = null"></div>
            <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6"
                x-transition:enter="transition duration-200" x-transition:enter-start="opacity-0 scale-95"
                x-transition:enter-end="opacity-100 scale-100">
                <div class="flex items-start gap-4">
                    <div class="w-10 h-10 rounded-full bg-red-100 grid place-items-center shrink-0">
                        <svg class="w-5 h-5 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                        </svg>
                    </div>
                    <div class="flex-1">
                        <h3 class="font-bold text-primary text-base">Hapus Data?</h3>
                        <p class="page-sub">Data berikut akan dihapus permanen:</p>
                        <p class="mt-2 text-sm text-primary font-medium bg-[#f8fafc] rounded-lg px-3 py-2 border border-line"
                            x-text="deleteText"></p>
                        <p class="text-xs text-red-500 mt-2">Tindakan ini tidak dapat dibatalkan.</p>
                    </div>
                </div>
                <div class="flex items-center justify-end gap-3 mt-6">
                    <button type="button" @click="deleteId = null"
                        class="px-4 py-2 border border-line text-muted rounded-xl text-sm font-semibold hover:text-primary hover:border-primary/30 transition">
                        Batal
                    </button>
                    <form method="POST" :action="`/admin/sekuritas-informasi/${deleteId}`">
                        @csrf @method('DELETE')
                        <button type="submit"
                            class="px-4 py-2 bg-red-600 text-white rounded-xl text-sm font-semibold hover:bg-red-700 transition">
                            Ya, Hapus
                        </button>
                    </form>
                </div>
            </div>
        </div>

    </div>
@endsection
