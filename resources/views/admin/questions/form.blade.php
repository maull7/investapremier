...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-primary">{{ isset($question) ? 'Edit Soal' : 'Tambah Soal Baru' }}</h1>
</div>
        ...
        <div>
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-start gap-3">
                    <div class="w-8 h-8 rounded-lg bg-primary/10 text-primary font-bold text-sm grid place-items-center shrink-0 mt-1">{{ $label }}</div>
                    <input type="hidden" name="options[{{ $i }}][label]" value="{{ $label }}">
                    <div class="flex-1">
                        <x-text-input type="text" name="options[{{ $i }}][option_text]"
                               value="{{ $oldOpt['option_text'] ?? ($opt->option_text ?? '') }}"
                               placeholder="Teks pilihan {{ $label }}"
                               class="w-full px-3 py-2 text-sm @error("options.$i.option_text") border-red-400 @enderror" />
                        <x-input-error :messages="$errors->get('options.'.$i.'.option_text')" class="mt-1 text-xs" />
                    </div>
                    <div class="w-24 shrink-0">
                        <x-text-input type="number" name="options[{{ $i }}][points]" min="1"
                               value="{{ $oldOpt['points'] ?? ($opt->points ?? '') }}"
                               placeholder="Poin"
                               class="w-full px-3 py-2 text-sm @error("options.$i.points") border-red-400 @enderror" />
                        <x-input-error :messages="$errors->get('options.'.$i.'.points')" class="mt-1 text-xs" />
                    </div>
                </div>
                @endforeach
            </div>
            <p class="text-xs text-muted mt-2">Poin tiap pilihan digunakan untuk menghitung profil investasi (total skor 8–32)</p>
        </div>
    </div>
