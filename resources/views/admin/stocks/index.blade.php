...
@section('content')
    <div x-data="{ deleteId: null, deleteText: '', showImport: false, showExtraction: false }">
                                    ...
                                    <td class="px-4 py-3 text-muted">{{ $batch->data_date?->format('d/m/Y') ?: '-' }}</td>
                                            ...
                                            <td class="px-4 py-3">
                                                <div class="flex items-center justify-end gap-1">
                                                    ...
                                                    <form method="POST" action="{{ route('admin.saham.extraction-batches.retry', $batch) }}">
                                                        @csrf
                        ...
                        <div class="p-4 border-b border-line">
                            <div class="flex flex-wrap items-center gap-2">
                                @if ($detailBatch->status === 'success')
                                    <form method="POST" action="{{ route('admin.saham.extraction-batches.save', $detailBatch) }}" class="flex items-center gap-2">
                                        @csrf
                                ...
                                @endif
                                <form method="POST" action="{{ route('admin.saham.extraction-batches.destroy', $detailBatch) }}">
                                    @csrf @method('DELETE')
                                ...
                                <tbody class="divide-y divide-line">
                                    @forelse ($detailBatch->stockDailyTransactions as $row)
                                        <tr>
                                            ...
                                            <td class="px-3 py-2 text-right">{{ number_format((float) $row->open, 2) }}</td>
                                            <td class="px-3 py-2 text-right">{{ number_format((float) $row->high, 2) }}</td>
                                            <td class="px-3 py-2 text-right">{{ number_format((float) $row->low, 2) }}</td>
                                            <td class="px-3 py-2 text-right">{{ number_format((float) $row->close, 2) }}</td>
                                            <td class="px-3 py-2 text-right">{{ $row->volume ? number_format($row->volume) : '-' }}</td>
        ...
        <div x-show="showExtraction" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4"
            ...
            <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6"
                ...
                <p class="text-muted text-sm mb-4">Semua kode saham akan diproses lewat Horizon. IDX Market dicoba lebih dulu jika endpoint tersedia, lalu fallback ke Yahoo Finance.</p>
                ...
                <form method="POST" action="{{ route('admin.saham.extraction-batches.store') }}">
                    ...
                    <div class="mt-4 rounded-xl border border-line bg-[#f8fafc] px-4 py-3 text-xs text-muted">
                        Data masuk tabel staging dulu. Database utama baru berubah setelah tombol Simpan ke Database diklik dari tab Hasil Ekstrak.
                    </div>
