...
@section('content')
    <div class="mb-6">
        <div class="flex items-center gap-2 text-sm text-muted mb-3">
            <a href="{{ route('admin.saham.index') }}" class="hover:text-primary transition">Daftar Saham</a>
    ...
    <form method="POST"
        action="{{ isset($stock) ? route('admin.saham.update', $stock) : route('admin.saham.store') }}"
        class="max-w-4xl">
        @csrf
        @if (isset($stock))
            @method('PUT')
