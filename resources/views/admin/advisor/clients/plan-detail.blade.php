...
@section('content')
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <p class="font-semibold text-sm">
                    {{ $plan->target_waktu_tahun ? $plan->target_waktu_tahun . ' tahun' : '-' }}</p>
            </div>
            <div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                ...
                @if ($plan->estimasi_biaya_saat_ini)
                    <div>
                        ...
                        <p class="font-semibold text-sm">Rp{{ number_format($plan->estimasi_biaya_saat_ini, 0, ',', '.') }}
                        </p>
                    </div>
                @endif
        ...
        <div class="bg-white rounded-2xl border border-line shadow-sm p-6 mb-6">
            ...
            <div class="space-y-3">
                    ...
                    <div class="bg-[#f8fafc] rounded-xl border border-line">
                        <div class="flex items-center justify-between p-3">
                            ...
                            <div class="text-right">
                                <p class="text-xs text-muted">
                                    {{ $item->jenis === 'Reksa Dana' ? number_format($item->nominal, 0, ',', '.') . ' UP' : ($item->jenis === 'Saham' ? number_format($item->nominal, 0, ',', '.') . ' lembar' : 'Rp' . number_format($item->nominal, 0, ',', '.')) }}
                                    × Rp{{ number_format($item->harga_akuisisi, 0, ',', '.') }}</p>
                                <p class="font-bold text-sm text-primary">Rp{{ number_format($item->nilai, 0, ',', '.') }}
                                </p>
                            </div>
                        </div>
                        <div class="border-t border-line">
                            ...
                            <div class="h-44 px-4 pb-4" style="display:none;">
                                <canvas class="portofolio-chart w-full h-full" data-jenis="{{ $item->jenis }}"
                ...
                <div class="flex items-center justify-between p-3 bg-accent/5 rounded-xl border border-accent/20">
                    ...
                    <span
                        class="font-bold text-lg text-primary">Rp{{ number_format($plan->portofolioItems->sum('nilai'), 0, ',', '.') }}</span>
                </div>
            </div>
        </div>
    @endif
    ...
    <div class="bg-white rounded-2xl border border-line shadow-sm p-6 mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div class="p-3 bg-[#f8fafc] rounded-xl border border-line">
                ...
                <p class="font-bold text-sm {{ $latestDana > 0 ? 'text-green-600' : 'text-muted' }}">
                    Rp{{ number_format($latestDana, 0, ',', '.') }}</p>
            </div>
            <div class="p-3 bg-[#f8fafc] rounded-xl border border-line">
                <p class="text-xs text-muted">Gap</p>
                <p class="font-bold text-sm {{ $kebutuhan - $latestDana > 0 ? 'text-red-600' : 'text-green-600' }}">
                    Rp{{ number_format(max(0, $kebutuhan - $latestDana), 0, ',', '.') }}</p>
            </div>
        </div>
        ...
        <div class="mb-2 flex items-center justify-between text-xs">
            ...
            <span
                class="font-semibold {{ $progressPct >= 100 ? 'text-green-600' : 'text-primary' }}">{{ $progressPct }}%</span>
        </div>
        <div class="w-full bg-gray-100 rounded-full h-3 overflow-hidden">
            <div class="h-full rounded-full transition-all duration-500 {{ $progressPct >= 100 ? 'bg-green-500' : 'bg-accent' }}"
                ...
                <div class="space-y-1.5">
                    @foreach ($plan->progressCheckins->take(5) as $c)
                        <div
                            ...
                            <span
                                class="font-semibold text-primary">Rp{{ number_format($c->dana_terkumpul, 0, ',', '.') }}</span>
                            @if ($c->catatan)
        ...
        @if (!empty($plan->ai_output['error']))
            <div class="p-6">
                <div class="flex items-start gap-3 p-4 bg-red-50 border border-red-200 rounded-xl">
                    <svg class="w-5 h-5 shrink-0 text-red-500 mt-0.5" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        @elseif (empty($plan->ai_output) && empty($plan->ai_narasi))
            <div class="p-6">
                <div class="flex items-start gap-3 p-4 bg-blue-50 border border-blue-200 rounded-xl">
                    <svg class="w-5 h-5 shrink-0 text-blue-500 mt-0.5" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="p-6 space-y-6">
                            ...
                            @foreach (['total_kebutuhan' => 'Total Kebutuhan', 'dana_saat_ini' => 'Dana Saat Ini', 'defisit' => 'Defisit', 'investasi_bulanan' => 'Investasi Bulanan'] as $key => $label)
                                @if (!empty($ai['analisis_keuangan'][$key]))
                                    <div class="p-3 bg-white rounded-xl border border-line">
                                        ...
                                        <p
                                            class="font-bold text-sm {{ $key === 'defisit' ? 'text-red-600' : 'text-primary' }}">
                                            {{ $ai['analisis_keuangan'][$key] }}</p>
                                    </div>
                                @endif
                            @endforeach
                        </div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                            @if (!empty($ai['proyeksi']['nilai_terkumpul']))
                                <div class="p-3 bg-green-50 rounded-xl border border-green-200">
                                    ...
                                    <p class="font-bold text-sm text-green-700">{{ $ai['proyeksi']['nilai_terkumpul'] }}
                                    </p>
                                </div>
                            @endif
                            ...
                            @if (!empty($ai['proyeksi']['gap_dana']))
                                <div
                                    class="p-3 {{ str_contains($ai['proyeksi']['gap_dana'], 'kurang') || str_contains($ai['proyeksi']['gap_dana'], 'defisit') ? 'bg-red-50 border-red-200' : 'bg-green-50 border-green-200' }} rounded-xl border">
                                    <p class="text-xs text-muted">Gap Dana</p>
                                    <p
                                        class="font-bold text-sm {{ str_contains($ai['proyeksi']['gap_dana'], 'kurang') || str_contains($ai['proyeksi']['gap_dana'], 'defisit') ? 'text-red-700' : 'text-green-700' }}">
                                        {{ $ai['proyeksi']['gap_dana'] }}</p>
                                </div>
                            @endif
                        </div>
                    </div>
                        ...
                        <ul class="space-y-2">
                            @foreach ($ai['rekomendasi_strategi'] as $item)
                                <li class="flex items-start gap-2 text-sm">
                                    <svg class="w-4 h-4 shrink-0 text-accent mt-0.5" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                            ...
                            @foreach ($ai['alokasi_aset'] as $item)
                                <div class="flex items-center gap-3 p-3 bg-white rounded-xl border border-line">
                                    <div class="w-16 text-center">
                                        <span
                                            class="inline-block px-2 py-1 rounded-lg bg-accent/10 text-accent font-bold text-xs">{{ $item['persentase'] ?? '' }}</span>
                                    </div>
                                    <div class="flex-1">
                            ...
                            @foreach ($ai['rekomendasi_portofolio'] as $efek)
                                ...
                                <div class="bg-white rounded-xl border border-line p-4">
                                    <div class="flex items-center justify-between mb-2">
                                        ...
                                        <span
                                            class="text-xs font-bold px-2.5 py-1 rounded-full {{ $aksiColor }}">{{ $aksi }}</span>
                                    </div>
                ...
                @if (!empty($ai['rekomendasi_investor']))
                    <div class="p-4 bg-accent/5 rounded-xl border border-accent/20">
                        <p class="text-xs text-muted font-semibold uppercase tracking-wide mb-1">Rekomendasi untuk Investor
                        </p>
    ...
    <div class="bg-white rounded-2xl border border-line shadow-sm overflow-hidden mb-6" x-data="{
        ...
        <div
            class="px-6 py-4 border-b border-line flex items-center justify-between bg-gradient-to-r from-blue-50 to-blue-100/50">
            <h3 class="font-bold text-primary text-sm flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                    ...
                    <input type="range" x-model="bulanan" min="100000" max="50000000" step="100000"
                        class="w-full accent-accent" @input="hitung()">
            ...
            <p class="text-xs text-muted mt-3">* Simulasi ini menggunakan bunga majemuk dan bersifat indikatif. Hasil
                sebenarnya dapat berbeda.</p>
        </div>
    </div>
