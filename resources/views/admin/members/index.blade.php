...
<div class="bg-white rounded-2xl border border-line overflow-hidden shadow-sm">
    ...
    <div class="px-6 py-4 border-t border-line flex items-center justify-between text-sm">
        ...
        <div class="flex items-center gap-1">
            ...
            <a href="{{ $url }}" class="w-8 h-8 rounded-lg grid place-items-center text-xs font-semibold transition {{ $page == $members->currentPage() ? 'bg-primary text-white' : 'text-muted hover:text-primary hover:bg-[#f1f5f9]' }}">{{ $page }}</a>
            @endforeach
