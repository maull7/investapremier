...
@section('content')
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-accent-teal/85">Unit Link</h1>
                            ...
                            @foreach ($unitLinks as $u)
                                <tr class="hover:bg-[#f8fafc] transition-colors">
                                    ...
                                    <td class="px-4 py-3 text-xs">
                                        <span
                                            class="px-2 py-0.5 rounded bg-primary/5 text-primary font-semibold">{{ $u->mata_uang ?: '-' }}</span>
                                    </td>
                                    <td class="px-4 py-3 text-right text-xs tabular-nums">
                                        {{ $u->median_price !== null ? number_format($u->median_price, 4, ',', '.') : '-' }}
                                    </td>
                                    <td class="px-4 py-3 text-right text-xs tabular-nums">
                                        {{ $u->buy_price !== null ? number_format($u->buy_price, 4, ',', '.') : '-' }}</td>
                                    <td class="px-4 py-3 text-right text-xs tabular-nums">
                                        {{ $u->sell_price !== null ? number_format($u->sell_price, 4, ',', '.') : '-' }}
                                    </td>
                                    <td class="px-4 py-3 text-right text-xs">
                                        {{ $u->last_update ? $u->last_update->format('d M Y') : '-' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
        ...
        <div class="table-card">
            ...
            @else
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-line">
                            @foreach ($hargaUnitLinks as $h)
                                <tr class="hover:bg-[#f8fafc] transition-colors">
                                    <td class="px-4 py-3 font-semibold text-primary text-sm">
                                        {{ $h->unitLink?->unit_link ?? '-' }}</td>
                                    <td class="px-4 py-3 text-xs tabular-nums">
                                        {{ $h->datetime ? $h->datetime->format('d/m/Y H:i') : '-' }}</td>
                                    <td class="px-4 py-3 text-right text-xs tabular-nums">
                                        {{ $h->harga_median !== null ? number_format($h->harga_median, 6, ',', '.') : '-' }}
                                    </td>
                                    <td class="px-4 py-3 text-right text-xs tabular-nums">
                                        {{ $h->sell_buy_low !== null ? number_format($h->sell_buy_low, 6, ',', '.') : '-' }}
                                    </td>
                                    <td class="px-4 py-3 text-right text-xs tabular-nums">
                                        {{ $h->sell_buy_high !== null ? number_format($h->sell_buy_high, 6, ',', '.') : '-' }}
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                ...
                @if ($hargaUnitLinks->hasPages())
                    <div class="px-6 py-4 border-t border-line flex items-center justify-between gap-4 text-sm">
                        <p class="text-muted text-xs">Menampilkan
                            {{ $hargaUnitLinks->firstItem() }}–{{ $hargaUnitLinks->lastItem() }} dari
                            {{ $hargaUnitLinks->total() }}</p>
                        <div class="flex items-center gap-1">
                            @if ($hargaUnitLinks->onFirstPage())
                                <span class="px-3 py-1.5 rounded-lg text-muted/40 text-xs cursor-not-allowed">← Prev</span>
                            @else
                                <a href="{{ $hargaUnitLinks->previousPageUrl() }}"
                                    class="px-3 py-1.5 rounded-lg text-muted hover:text-primary hover:bg-[#f1f5f9] transition text-xs">←
                                    Prev</a>
                            @endif
                            @php
                                $cur = $hargaUnitLinks->currentPage();
                                $last = $hargaUnitLinks->lastPage();
                                $s = max(1, $cur - 2);
                                $e = min($last, $cur + 2);
                            @endphp
                            @if ($s > 1)
                                <a href="{{ $hargaUnitLinks->url(1) }}"
                                    class="w-8 h-8 rounded-lg grid place-items-center text-xs font-semibold text-muted hover:text-primary hover:bg-[#f1f5f9] transition">1</a>
                                @if ($s > 2)
                                    <span class="px-1 text-muted text-xs">…</span>
                                @endif
                            ...
                            @foreach ($hargaUnitLinks->getUrlRange($s, $e) as $page => $url)
                                <a href="{{ $url }}"
                                    class="w-8 h-8 rounded-lg grid place-items-center text-xs font-semibold transition {{ $page == $cur ? 'bg-accent-teal text-white' : 'text-muted hover:text-primary hover:bg-[#f1f5f9]' }}">{{ $page }}</a>
                            @endforeach
                            @if ($e < $last)
                                @if ($e < $last - 1)
                                    <span class="px-1 text-muted text-xs">…</span>
                                @endif
                                <a href="{{ $hargaUnitLinks->url($last) }}"
                                    class="w-8 h-8 rounded-lg grid place-items-center text-xs font-semibold text-muted hover:text-primary hover:bg-[#f1f5f9] transition">{{ $last }}</a>
                            @endif
                            @if ($hargaUnitLinks->hasMorePages())
                                <a href="{{ $hargaUnitLinks->nextPageUrl() }}"
                                    class="px-3 py-1.5 rounded-lg text-muted hover:text-primary hover:bg-[#f1f5f9] transition text-xs">Next
                                    →</a>
                            @else
                                <span class="px-3 py-1.5 rounded-lg text-muted/40 text-xs cursor-not-allowed">Next →</span>
