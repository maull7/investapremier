...
@section('body')
<div x-data="{ sidebarOpen: false, menuMasterOpen: {{ request()->routeIs('admin.dashboard') || request()->routeIs('admin.questions.*') || request()->routeIs('admin.members.*') ? 'true' : 'false' }} }" class="flex h-screen overflow-hidden">
    {{-- Overlay mobile --}}
    ...
    <aside class="hidden lg:flex w-64 bg-primary text-white flex-col shrink-0">
        ...
        <nav class="flex-1 py-4 px-3 space-y-1 text-sm">
            <button type="button" @@click="menuMasterOpen = !menuMasterOpen"
               class="w-full flex items-center justify-between gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('admin.dashboard') || request()->routeIs('admin.questions.*') || request()->routeIs('admin.members.*') ? 'bg-white/10 font-semibold' : 'text-white/70 hover:bg-white/5 hover:text-white' }}">
                ...
                <svg class="w-4 h-4 transition-transform" :class="{ 'rotate-180': menuMasterOpen }" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
            </button>
            <div x-show="menuMasterOpen" x-transition class="space-y-1 pl-3">
                <a href="{{ route('admin.dashboard') }}"
                   class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('admin.dashboard') ? 'bg-white/10 font-semibold text-white' : 'text-white/70 hover:bg-white/5 hover:text-white' }}">
                    <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7"/></svg>
                    Dashboard
                </a>
                <a href="{{ route('admin.questions.index') }}"
                   class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('admin.questions.*') ? 'bg-white/10 font-semibold text-white' : 'text-white/70 hover:bg-white/5 hover:text-white' }}">
                    <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7"/></svg>
                    Soal Kuis
                </a>
                <a href="{{ route('admin.members.index') }}"
                   class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('admin.members.*') ? 'bg-white/10 font-semibold text-white' : 'text-white/70 hover:bg-white/5 hover:text-white' }}">
                    <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3"/></svg>
                    Pendaftaran Member
                </a>
            </div>
            <a href="{{ route('admin.analisa.index') }}"
               class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('admin.analisa.*') ? 'bg-white/10 font-semibold' : 'text-white/70 hover:bg-white/5 hover:text-white' }}">
            ...
            <a href="/horizon" target="_blank"
               class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-white/70 hover:bg-white/5 hover:text-white">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 3H5a2 2 0 00-2 2v4m6-6h10a2 2 0 012 2v4M9 3v18m0 0h10a2 2 0 002-2V9M9 21H5a2 2 0 01-2-2V9m0 0h18"/></svg>
                Queue Monitor
            </a>
        ...
        <header class="h-16 bg-white border-b border-line flex items-center justify-between gap-4 px-4 lg:px-6 shrink-0">
            ...
            <div class="flex items-center gap-2 ml-auto">
                <div class="flex items-center gap-2 sm:gap-3 text-sm">
                    ...
                    <div class="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-primary text-white grid place-items-center text-xs font-bold uppercase shrink-0">{{ substr(Auth::user()->name, 0, 2) }}</div>
                </div>
