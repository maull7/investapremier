    ...
    <div x-data="{
            ...
            <nav class="flex-1 py-4 px-3 space-y-1 text-sm overflow-y-auto">
                ...
                <div>
                    <button type="button" @click="layananOpen = !layananOpen"
                        ...
                        <svg class="w-4 h-4 transition-transform" :class="{ 'rotate-180': layananOpen }" fill="none"
                ...
                <a href="{{ route('user.notifications.index') }}"
                    ...
                    @if ($__unread > 0)
                        <span
                            class="ml-auto text-[10px] font-bold text-white bg-red-500 px-2 py-0.5 rounded-full">{{ $__unread > 99 ? '99+' : $__unread }}</span>
                    @endif
                </a>
        ...
        <aside x-show="sidebarOpen" x-cloak
            ...
            <nav class="flex-1 py-4 px-3 space-y-1 text-sm overflow-y-auto">
                ...
                {{-- Manajer Investasi --}}
                <a href="{{ route('user.investment-managers.index') }}" @@click="sidebarOpen = false"
                ...
                @else
                    <a href="{{ route('user.clients.requests.index') }}" @@click="sidebarOpen = false"
                        class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('user.clients.requests.*') ? 'bg-green-50 text-green-700 font-semibold' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
                        <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
                ...
                <a href="{{ route('user.notifications.index') }}" @@click="sidebarOpen = false"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('user.notifications.*') ? 'bg-green-50 text-green-700 font-semibold' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
                    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                    ...
                    @if ($__unreadM > 0)
                        <span
                            class="ml-auto text-[10px] font-bold text-white bg-red-500 px-2 py-0.5 rounded-full">{{ $__unreadM > 99 ? '99+' : $__unreadM }}</span>
                    @endif
                </a>
            ...
            <header
                ...
                <div class="flex items-center gap-2 ml-auto">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit"
                            class="flex items-center gap-1.5 px-2.5 py-2 rounded-lg text-gray-500 hover:text-gray-900 hover:bg-gray-100 transition text-sm">
