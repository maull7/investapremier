...
<div class="bg-white rounded-xl border border-line p-6 space-y-4">
    <h3 class="font-semibold text-primary">Neraca (Balance Sheet) <span class="text-xs font-normal text-muted">— dalam
            juta Rupiah (atau sesuai mata uang)</span></h3>

    <div class="flex flex-row items-center gap-2 align-baseline">
        <div>
        ...
        <div class="space-y-3">
            ...
            @foreach ([['cash_equivalents', 'Kas & Setara Kas'], ['account_receivable', 'Piutang Usaha'], ['inventories', 'Persediaan'], ['other_current_asset', 'Aset Lancar Lainnya'], ['current_asset', 'Total Aset Lancar']] as [$name, $label])
                <div class="flex items-center gap-2">
                    ...
                    <input type="number" name="{{ $name }}" x-model="{{ $name }}" step="0.01"
                        value="{{ old($name) }}"
