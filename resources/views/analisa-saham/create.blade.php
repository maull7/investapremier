        ...
        <div class="bg-white rounded-xl border border-line p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="sektor" class="block w-full border-gray-300 rounded-lg shadow-sm focus:border-primary focus:ring focus:ring-primary/20 text-sm">
                        ...
                        @foreach(['Perbankan','Keuangan','Consumer Goods','Energi','Infrastruktur','Industri Dasar','Perkebunan','Properti','Teknologi','Transportasi','Telekomunikasi','Pertambangan','Lainnya'] as $s)
                            <option value="{{ $s }}" {{ old('sektor') === $s ? 'selected' : '' }}>{{ $s }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
        ...
        parsePdf() {
            ...
            fetch(parsePdfUrl, { method: 'POST', headers: { 'Accept': 'application/json' }, body: fd })
                .then(async r => {
                    ...
                    if (d.sektor) { const sel = document.querySelector('[name="sektor"]'); if (sel) { [...sel.options].forEach(o => { if (o.value === d.sektor) o.selected = true; }); } }
