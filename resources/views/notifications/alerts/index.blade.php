...
@section('content')
    <div>
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-accent-teal/85">Alert Harga Saham</h1>
                            ...
                            @foreach ($alerts as $alert)
                                <tr>
                                    ...
                                    <td class="font-semibold">Rp
                                        {{ number_format((float) $alert->target_price, 2, ',', '.') }}</td>
                                    <td>
                                    ...
                                    <td>
                                        @if ($alert->is_active)
                                            <span
                                                class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-bold bg-green-100 text-green-700">Aktif</span>
                                        @else
                                            <span
                                                class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-bold bg-gray-100 text-gray-500">Nonaktif</span>
                                        @endif
                                        @if ($alert->repeat)
                                            <span
                                                class="ml-1 inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-bold bg-blue-100 text-blue-700">Repeat</span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="text-right">
                                        <div class="inline-flex items-center gap-1">
                                            <form method="POST" action="{{ route('user.price-alerts.toggle', $alert) }}">
                                                @csrf
                                                <button type="submit" class="btn-icon btn-secondary"
                                            ...
                                            <a href="{{ route('user.price-alerts.edit', $alert) }}"
                                                class="btn-icon btn-secondary" title="Edit">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            ...
                                            <button type="button" onclick="confirmDelete(this)"
                                                ...
                                                data-label="Alert {{ $alert->kode_efek }}" class="btn-icon btn-danger"
                                                title="Hapus">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
