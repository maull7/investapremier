        ...
        <div class="mb-6 flex items-center justify-between">
            <div>
                ...
                <p class="page-sub">
                    ...
                    @if ($unreadCount > 0)
                        <span class="ml-2 inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-bold bg-red-100 text-red-600">{{ $unreadCount }} belum dibaca</span>
                    @endif
                </p>
            </div>
        ...
        <div class="table-card">
            ...
            @if ($notifications->isEmpty())
                <div class="empty-state">
                    ...
                    <a href="{{ route('user.price-alerts.create') }}" class="btn-primary mt-4 inline-flex">
                    ...
                    @foreach ($notifications as $n)
                        ...
                        <li class="px-5 py-4 hover:bg-gray-50 transition {{ $n->read_at ? '' : 'bg-green-50/30' }}">
                            <div class="flex items-start gap-4">
                                <div class="w-10 h-10 rounded-full grid place-items-center shrink-0 text-white {{ $isBelow ? 'bg-red-500' : 'bg-green-600' }}">
                                    @if ($isBelow)
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        <span class="font-semibold text-sm text-gray-900">{{ $data['title'] ?? 'Notifikasi' }}</span>
                                        @if (! $n->read_at)
                                            <span class="text-[10px] font-bold uppercase tracking-wide bg-red-100 text-red-600 px-2 py-0.5 rounded-full">Baru</span>
                                        @endif
                                    </div>
                                    ...
                                    <div class="flex items-center gap-3 mt-2 text-xs text-gray-400">
                                        ...
                                        @if ($url)
                                            <span>·</span>
                                            <a href="{{ $url }}" class="text-green-700 hover:text-green-900 font-semibold">Lihat detail →</a>
                                        @endif
                                    </div>
                                </div>
