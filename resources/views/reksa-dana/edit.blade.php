...
@section('content')
    <div class="max-w-5xl" x-data="editForm()">
        ...
        <form method="POST" action="{{ route('user.reksa-dana.update', $reksaDana) }}" class="space-y-6">
            @csrf @method('PUT')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-text-input id="nama_reksa_dana" name="nama_reksa_dana" type="text" class="mt-1 block w-full"
                            value="{{ old('nama_reksa_dana', $reksaDana->nama_reksa_dana) }}" required />
                    ...
                    <div>
                        ...
                        <select id="jenis_reksa_dana" name="jenis_reksa_dana" required
                            ...
                            @foreach ($jenisOptions as $j)
                                <option value="{{ $j }}"
                                    {{ old('jenis_reksa_dana', $reksaDana->jenis_reksa_dana) === $j ? 'selected' : '' }}>
                                    {{ $j }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <x-text-input id="total_aum" name="total_aum" type="number" step="0.01"
                            class="mt-1 block w-full" value="{{ old('total_aum', $reksaDana->total_aum) }}" />
                    ...
                    <div>
                        ...
                        <x-text-input id="total_marcap_10_efek" name="total_marcap_10_efek" type="number" step="0.01"
                            class="mt-1 block w-full"
                            value="{{ old('total_marcap_10_efek', $reksaDana->total_marcap_10_efek) }}" />
                    ...
                    <div>
                        ...
                        <x-text-input id="tanggal_data" name="tanggal_data" type="date" class="mt-1 block w-full"
                            value="{{ old('tanggal_data', $reksaDana->tanggal_data?->format('Y-m-d')) }}" />
            ...
            <div class="bg-white rounded-xl border border-line p-6 space-y-3">
                <div class="flex items-center justify-between">
                    ...
                    <button type="button" @click="addRow('sektor')" class="text-xs text-primary hover:underline">+ Tambah
                        Baris</button>
                </div>
                <div class="space-y-2">
                    <template x-for="(row, i) in sektor" :key="i">
                        <div class="flex gap-2 items-center">
                            ...
                            <input type="number" :name="`sektor[${i}][bobot]`" x-model="row.bobot" placeholder="Bobot %"
                                step="0.01"
            ...
            <div class="bg-white rounded-xl border border-line p-6 space-y-3">
                <div class="flex items-center justify-between">
                    ...
                    <button type="button" @click="addRow('efek')" class="text-xs text-primary hover:underline">+ Tambah
                        Baris</button>
                </div>
            ...
            <div class="bg-white rounded-xl border border-line p-6 space-y-3">
                <div class="flex items-center justify-between">
                    ...
                    <button type="button" @click="addRow('kinerja')" class="text-xs text-primary hover:underline">+
                        Tambah Baris</button>
                </div>
                <div class="space-y-2">
            ...
            <div class="bg-white rounded-xl border border-line p-6 space-y-3">
                <div class="flex items-center justify-between">
                    ...
                    <button type="button" @click="addRow('obligasi')" class="text-xs text-primary hover:underline">+
                        Tambah Baris</button>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-line">
                            <template x-for="(row, i) in obligasi" :key="i">
                                <tr>
                                    <td class="px-1 py-1"><input type="text" :name="`obligasi[${i}][kode_obligasi]`"
                                            x-model="row.kode_obligasi" placeholder="FR0091"
                                            class="w-24 border-gray-300 rounded text-xs px-2 py-1.5 focus:border-primary focus:ring focus:ring-primary/20" />
                                    </td>
                                    <td class="px-1 py-1"><input type="text" :name="`obligasi[${i}][nama_obligasi]`"
                                            x-model="row.nama_obligasi" placeholder="Nama Obligasi"
                                    ...
                                    <td class="px-1 py-1"><button type="button" @click="removeRow('obligasi', i)"
                                            class="text-red-400 hover:text-red-600 text-xs">✕</button></td>
                                </tr>
                            </template>
                        </tbody>
            ...
            <div class="bg-white rounded-xl border border-line p-6 space-y-3">
                <div class="flex items-center justify-between">
                    ...
                    <button type="button" @click="addRow('bank')" class="text-xs text-primary hover:underline">+ Tambah
                        Baris</button>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-line">
                            <template x-for="(row, i) in bank" :key="i">
                                <tr>
                                    ...
                                    <td class="px-1 py-1">
                                        <select :name="`bank[${i}][klasifikasi_risiko]`" x-model="row.klasifikasi_risiko"
                                            class="border-gray-300 rounded text-xs px-2 py-1.5 focus:border-primary focus:ring focus:ring-primary/20">
                                            <option value="">-</option>
