    ...
    <div x-data="{
        ...
        openEdit(section) {
            ...
            if (section === 'ringkasan') {
                ...
                this.editData = {
                    ...
                    sharpe_ratio_1y: @js($fund->sharpe_ratio_1y),
                    sharpe_ratio_3y: @js($fund->sharpe_ratio_3y),
                    sharpe_ratio_5y: @js($fund->sharpe_ratio_5y),
                    stdev_1y: @js($fund->stdev_1y),
                    stdev_3y: @js($fund->stdev_3y),
                    stdev_5y: @js($fund->stdev_5y),
                    beta_1y: @js($fund->beta_1y),
                    beta_3y: @js($fund->beta_3y),
                    beta_5y: @js($fund->beta_5y),
                    max_drawdown_1y: @js($fund->max_drawdown_1y),
                    max_drawdown_3y: @js($fund->max_drawdown_3y),
                    max_drawdown_5y: @js($fund->max_drawdown_5y),
                };
            } else if (section === 'biaya') {
                this.editData = {
                    subscription_fee: @js($fund->subscription_fee),
                    redemption_fee: @js($fund->redemption_fee),
                    switching_fee: @js($fund->switching_fee),
                    management_fee: @js($fund->management_fee),
                    custodian_fee: @js($fund->custodian_fee),
                    expense_ratio: @js($fund->expense_ratio),
                    investment_manager_fee: @js($fund->investment_manager_fee),
                    minimum_subscription: @js($fund->minimum_subscription),
                    minimum_topup: @js($fund->minimum_topup),
                    minimum_redemption: @js($fund->minimum_redemption),
                };
        ...
        <div class="mb-6">
            <div class="flex items-center gap-2 text-sm text-muted mb-3">
                <a href="{{ route('user.reksa-dana.index') }}" class="hover:text-primary transition">Daftar Reksa Dana</a>
            ...
            <div class="flex items-center gap-3 mt-3 flex-wrap">
                ...
                <select x-model="portfolioMonth" @change="loadPortfolioData"
                ...
                <select x-model="portfolioYear" @change="loadPortfolioData"
                    class="border border-line rounded-lg px-3 py-1.5 text-xs text-muted">
        ...
        <div class="grid grid-cols-2 md:grid-cols-6 gap-4 mb-6">
            <div class="bg-white rounded-xl border border-line p-4">
                ...
                <p class="text-sm font-bold text-primary">
                    {{ $latestNav ? number_format($latestNav->nab_per_unit, 4, ',', '.') : ($fund->nab_per_unit ? number_format($fund->nab_per_unit, 4, ',', '.') : '—') }}
                </p>
            </div>
            {{-- <div class="bg-white rounded-xl border border-line p-4">
        ...
        <p class="text-sm font-bold {{ $returnDaily !== null ? ($returnDaily >= 0 ? 'text-green-600' : 'text-red-600') : 'text-muted' }}">{{ $returnDaily !== null ? number_format($returnDaily, 2, ',', '.') . '%' : '—' }}</p>
            ...
            <div class="bg-white rounded-xl border border-line p-4">
                ...
                <p
                    class="text-sm font-bold {{ $returnMonthly !== null ? ($returnMonthly >= 0 ? 'text-green-600' : 'text-red-600') : 'text-muted' }}">
                    {{ $returnMonthly !== null ? number_format($returnMonthly, 2, ',', '.') . '%' : '—' }}</p>
            </div>
            <div class="bg-white rounded-xl border border-line p-4">
                <p class="text-xs text-muted mb-1">Return Tahunan</p>
                <p
                    class="text-sm font-bold {{ $returnYearly !== null ? ($returnYearly >= 0 ? 'text-green-600' : 'text-red-600') : 'text-muted' }}">
                    {{ $returnYearly !== null ? number_format($returnYearly, 2, ',', '.') . '%' : '—' }}</p>
            </div>
            <div class="bg-white rounded-xl border border-line p-4">
                <p class="text-xs text-muted mb-1">AUM</p>
                <p class="text-sm font-bold text-primary">
                    {{ $latestNav && $latestNav->aum ? 'Rp' . number_format($latestNav->aum, 0, ',', '.') : ($fund->aum ? 'Rp' . number_format($fund->aum, 0, ',', '.') : '—') }}
                </p>
            </div>
            <div class="bg-white rounded-xl border border-line p-4">
                <p class="text-xs text-muted mb-1">Unit Penyertaan</p>
                <p class="text-sm font-bold text-primary">
                    {{ $latestNav && $latestNav->unit_participation ? number_format($latestNav->unit_participation, 0, ',', '.') : ($fund->total_unit ? number_format($fund->total_unit, 0, ',', '.') : '—') }}
                </p>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-white rounded-2xl border border-line shadow-sm overflow-hidden">
                    ...
                    <div class="divide-y divide-line">
                        ...
                        @if ($fund->isin_code)
                            <div class="px-6 py-3.5 flex items-start gap-4"><span
                                    class="text-xs font-semibold text-muted w-36 shrink-0">ISIN Code</span><span
                ...
                <div class="bg-white rounded-2xl border border-line shadow-sm overflow-hidden">
                    ...
                    <div class="divide-y divide-line">
                        <div class="px-6 py-3.5 flex items-start gap-4"><span
                                ...
                                class="text-sm font-bold text-primary">{{ $latestNav ? number_format($latestNav->nab_per_unit, 4, ',', '.') : ($fund->nab_per_unit ? number_format($fund->nab_per_unit, 4, ',', '.') : '—') }}</span>
                        </div>
                        ...
                        <div class="px-6 py-3.5 flex items-start gap-4"><span
                                class="text-xs font-semibold text-muted w-36 shrink-0">Return Bulanan</span><span
                                class="text-sm font-bold {{ $returnMonthly !== null ? ($returnMonthly >= 0 ? 'text-green-600' : 'text-red-600') : 'text-muted' }}">{{ $returnMonthly !== null ? number_format($returnMonthly, 2, ',', '.') . '%' : '—' }}</span>
                        </div>
                        <div class="px-6 py-3.5 flex items-start gap-4"><span
                                class="text-xs font-semibold text-muted w-36 shrink-0">Return Tahunan</span><span
                                class="text-sm font-bold {{ $returnYearly !== null ? ($returnYearly >= 0 ? 'text-green-600' : 'text-red-600') : 'text-muted' }}">{{ $returnYearly !== null ? number_format($returnYearly, 2, ',', '.') . '%' : '—' }}</span>
                        </div>
                        <div class="px-6 py-3.5 flex items-start gap-4"><span
                                class="text-xs font-semibold text-muted w-36 shrink-0">AUM</span><span
                                class="text-sm font-bold text-primary">{{ $latestNav && $latestNav->aum ? 'Rp' . number_format($latestNav->aum, 0, ',', '.') : ($fund->aum ? 'Rp' . number_format($fund->aum, 0, ',', '.') : '—') }}</span>
                        </div>
                        <div class="px-6 py-3.5 flex items-start gap-4"><span
                                class="text-xs font-semibold text-muted w-36 shrink-0">Unit Penyertaan</span><span
                                class="text-sm font-bold text-primary">{{ $latestNav && $latestNav->unit_participation ? number_format($latestNav->unit_participation, 0, ',', '.') : ($fund->total_unit ? number_format($fund->total_unit, 0, ',', '.') : '—') }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-line shadow-sm overflow-hidden">
                ...
                @php
                    $hasFeeData =
                        ...
                        $fund->minimum_topup ||
                        $fund->minimum_redemption ||
                        $fund->expense_ratio ||
                        $fund->investment_manager_fee;
                @endphp
                @if ($hasFeeData)
                    <div class="divide-y divide-line">
        ...
        {{-- TAB: PORTOFOLIO --}}
        <div x-show="tab === 'portofolio'" x-cloak x-init="portfolioMonth = portfolioMonth || new Date().getMonth() + 1;
        portfolioYear = portfolioYear || new Date().getFullYear();
        loadPortfolioData()">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <div class="bg-white rounded-2xl border border-line shadow-sm p-5">
                                <h3 class="font-bold text-primary text-sm mb-4">Asset Allocation
                                    ({{ $latestAa->period_date->format('M Y') }})</h3>
                        ...
                        @if ($latestAa = $aaTimeline->last())
                            new Chart(document.getElementById('chartAaPie'), {
                                ...
                                data: {
                                    ...
                                    datasets: [{
                                        data: [{{ $latestAa->equity_percent ?? 0 }},
                                            {{ $latestAa->bond_percent ?? 0 }},
                                            {{ $latestAa->money_market_percent ?? 0 }},
                                            {{ $latestAa->cash_percent ?? 0 }}
                                        ],
        ...
        {{-- TAB: PDF PROSPEKTUS - REMOVED FOR USER --}}

        {{-- TAB: PDF FFS - REMOVED FOR USER --}}
    ...
    <script>
        function documentTabData(defaultDocType) {
            return {
                ...
                ffsError: {},
                pageContentCache: {},
                partitionsByDoc: @json(
                    $fund->documents->mapWithKeys(fn($d) => [
